...
public class LedgerAiAnalysisService {
    ...
    public LedgerAiAnalysisResponse analyze(Long userId, LedgerAiAnalysisRequest request) {
        ...
        try {
            ...
            failedHistory.setSummary("AI ?????Β?レ름???????椰??????????ㅼ뒩??);
    ...
    public LedgerAiAnalysisHistoryPageResponse getHistories(
        ...
        if (createdFrom != null && createdTo != null && createdFrom.isAfter(createdTo)) {
            throw new BadRequestException("?????Β?レ름??????????????곕툠???汝뷴젆?琉???????轅붽틓???壤굿??걜??? ???????욱떌????????꾨굔???????????????源낆┰?????????곸죩.");
        }
    ...
    public LedgerAiAnalysisHistoryDetailResponse getHistory(Long userId, Long historyId) {
        ...
        LedgerAiAnalysisHistory history = historyRepository.findByIdAndOwnerId(historyId, userId)
                .orElseThrow(() -> new NotFoundException("AI ?????Β?レ름??????????????곕툠???耀붾굝????????????????源낆┰?????????곸죩."));
    ...
    private DateRange validateCustomRange(LocalDate from, LocalDate to, String label) {
        if (from == null || to == null) {
            throw new BadRequestException(label + "????????轅붽틓???壤굿??걜??????怨쀫뎐?????????욱떌????關?쒎첎?嫄????????獄쏅챶留????轅붽틓??????");
        }
        if (from.isAfter(to)) {
            throw new BadRequestException(label + "?????轅붽틓???壤굿??걜??? ???????욱떌????????꾨굔???????????????源낆┰?????????곸죩.");
        }
        long days = ChronoUnit.DAYS.between(from, to) + 1;
        if (days > MAX_CUSTOM_RANGE_DAYS) {
            throw new BadRequestException(label + "?? ?耀붾굝??????鶯? 1?????ш낄???????궰??????繹먮끍????????壤굿??Β??????????????????곸죩.");
        }
    ...
    private LedgerAiAnalysisReportResponse buildFallbackReport(AnalysisPlan plan, AnalysisDataset dataset) {
        ...
        return new LedgerAiAnalysisReportResponse(
                keySummary,
                fullReport,
                "????壤굿??Β????????????곸궔???????꿔꺂????裕뼘???耀붾굝?????????????? " + formatWon(averageExpense) + "????? ??" + expenseCount + "?耀붾굝梨루땟??????耀붾굝?????????????源낆┸????????????????????傭?끆???????????????????곸죩.",
                notableSpending,
                regularSpending,
                abnormalSpending,
                topPaymentMethod,
    ...
    private String buildKeySummary(AnalysisPlan plan, AnalysisDataset dataset, BigDecimal totalExpense, BigDecimal averageExpense, long expenseCount, List<String> comparisonFocus) {
        if (expenseCount == 0) {
            return "????壤굿??Β????????????곸궔???????耀붾굝?????????????源낆┸?????????源낆┸??????????????????Β?レ름?????????썹땟洹욌뙀???????????";
        }
        String range = plan.primaryRange().from() + " ~ " + plan.primaryRange().to();
        if (plan.mode() == LedgerAiAnalysisMode.COMPARISON && dataset.comparisonOverview() != null) {
            ...
            return range + " ?耀붾굝?????????????? " + formatWon(totalExpense) + "????????????????????곸궔??????" + formatSignedWon(delta)
                    + "(" + formatPercentChange(delta, compareExpense) + ")??????筌?캉?? "
                    + (comparisonFocus.isEmpty() ? "??????????????? ??????꾨굴?? ?????Β?レ름?????????獄쏅챶留????轅붽틓??????" : comparisonFocus.get(0));
        }
        return range + " ?耀붾굝?????????????? ??" + formatWon(totalExpense) + ", ????꿔꺂????裕뼘??" + formatWon(averageExpense)
                + "??????筌?캉?? " + expenseCount + "?耀붾굝梨루땟??????耀붾굝????????????????????????????????????????????????癒꺜??????ル뒌??亦껋꼦維뽬뜎?????????? ??饔낅떽?????????????????????곸죩.";
    }
    ...
    private String buildFullReport(
        ...
        report.append("?????Β?レ름?????????????곸궔??? ").append(plan.primaryRange().from()).append("?????堉온???").append(plan.primaryRange().to()).append("????濡?씀?濾????????筌?캉?? ")
                .append("???耀붾굝?????????????? ").append(formatWon(totalExpense)).append(", ????꿔꺂????裕뼘???耀붾굝?????????????? ").append(formatWon(averageExpense))
                .append("??????耀붾굝?????????????源낆┸??? ").append(expenseCount).append("?耀붾굝梨루땟????????????곸죩. ");
        if (plan.mode() == LedgerAiAnalysisMode.COMPARISON && dataset.comparisonOverview() != null) {
            report.append("???????????????곸궔??").append(plan.comparisonRange().from()).append("?????堉온???").append(plan.comparisonRange().to()).append("????濡?씀?濾?????耀붾굝?????????????? ")
                    .append(formatWon(dataset.comparisonOverview().expense())).append("?? ??????????Β?レ름?????????? ?耀붾굝????????耀붾굝???壤굿?怨쀫ぁ???筌??⑤슦??????鶯ㅺ동???????耀붾굝???壤굿?怨쀫ぁ???筌????????Β?レ름??????????⑤떯?????饔낅떽???????????黎앸럽????룸돥??????汝뷴젆?琉???????????곸죩. ");
    ...
    private List<String> buildNotableSpending(AnalysisDataset dataset) {
        ...
        dataset.categoryBreakdown().stream()
                .limit(3)
                .forEach(item -> items.add(categoryLabel(item.groupName(), item.detailName()) + "??" + formatWon(item.totalAmount()) + "(" + item.entryCount() + "????????????????????곸죩."));
        dataset.topExpenses().stream()
                .limit(3)
                .forEach(item -> items.add("?????????耀붾굝????????????獄쏅챶留??? " + item.entryDate() + " " + item.title() + " " + formatWon(item.amount()) + "."));
        return items;
    }
    ...
    private List<String> buildRegularSpending(AnalysisDataset dataset) {
        return dataset.categoryBreakdown().stream()
                ...
                .limit(3)
                .map(item -> categoryLabel(item.groupName(), item.detailName()) + "?? " + item.entryCount() + "?耀붾굝梨루땟?????????????밸븶筌믩끃????????????????? ?????븍툖???紐꽺??쇻돘壤????????곸죩.")
                .toList();
    }
    ...
    private List<String> buildAbnormalSpending(AnalysisDataset dataset, BigDecimal averageExpense) {
        ...
        dataset.topExpenses().stream()
                ...
                .limit(3)
                .forEach(item -> items.add(item.entryDate() + "??" + item.title() + "(" + formatWon(item.amount()) + ")?? ????꿔꺂????裕뼘???耀붾굝???????????????????????????耀붾굝??????????????????????곸죩."));
        if (items.isEmpty() && !dataset.topExpenses().isEmpty()) {
            ExpenseEntryPayload top = dataset.topExpenses().get(0);
            items.add("?????ル뒌???????????믩베????耀붾굝?????????????? " + top.entryDate() + " " + top.title() + " " + formatWon(top.amount()) + "??????筌?캉??");
        }
        return items;
    }
    ...
    private List<String> buildSubscriptionInsights(List<RecurringExpenseCandidatePayload> recurringCandidates) {
        List<String> subscriptions = recurringCandidates.stream()
                ...
                .limit(5)
                .map(item -> item.title() + "?? " + item.occurrenceCount() + "???????밸븶筌믩끃??????轅붽틓???????????壤굿??ル만???耀붾굝????????????獄쏅챶留????????筌?캉??")
                .toList();
        return subscriptions.isEmpty() ? List.of("?耀붾굝????癲ル슢??㎖?밤뀋????????壤굿??ル만???????밸븶筌믩끃?????耀붾굝?????????????? ??饔낅떽??????????? ?????濚밸Ŧ援???????") : subscriptions;
    }
    ...
    private List<String> buildFixedExpenseInsights(List<RecurringExpenseCandidatePayload> recurringCandidates) {
        if (recurringCandidates.isEmpty()) {
            return List.of("???????살퓢?????轅붽틓?????룰퀣維?? ?????Β?レ름??????? ??汝뷴젆?琉??????黎앸럽????????????2??????????????밸븶筌믩끃???????????????????????獄쏅챶留??????饔낅떽??????????? ?????濚밸Ŧ援???????");
        }
        return recurringCandidates.stream()
                .limit(5)
                .map(item -> item.title() + "?? " + item.firstDate() + "?????堉온???" + item.lastDate() + "????濡?씀?濾?? " + item.occurrenceCount()
                        + "???????밸븶筌믩끃??????????????룸챷援????" + formatWon(item.totalAmount()) + "??????筌?캉??")
                .toList();
    }
    ...
    private List<String> buildImprovementActions(AnalysisPlan plan, AnalysisDataset dataset, List<RecurringExpenseCandidatePayload> recurringCandidates, List<String> comparisonFocus) {
        ...
        dataset.categoryBreakdown().stream().findFirst()
                .ifPresent(item -> actions.add(categoryLabel(item.groupName(), item.detailName()) + " ?耀붾굝???????????????????????? ??????????????ル뒌??亦껋꼦維뽬뜎??????????????????????ル뒌?? ?????ル뒌????????????녎??"));
        if (!recurringCandidates.isEmpty()) {
            actions.add("?????밸븶筌믩끃?????耀붾굝????????????獄쏅챶留????????μ떜媛?걫?繹먃???????????????살퐵????獄쏅챶留??泳?뿀???곌떽?깅즿????繹먮끍?? ??饔낅떽????????????嚥▲굧???? ?????Β?ル윲?????????????? ??? ???????汝뷴젆?琉??????????밸쫫?????욱룏??????⑥ル????????汝뷴젆?琉??????壤굿??Β???饔낅떽??????");
        }
        if (plan.mode() == LedgerAiAnalysisMode.COMPARISON && !comparisonFocus.isEmpty()) {
            actions.add("???????????????곸궔??????????耀붾굝???壤굿?怨쀫ぁ???筌????????Β?レ름??????????⑤떯???????μ떜媛?걫?????????????곸궔???????????????곗뒩筌????????먃??????嶺?筌??????怨멸텛??????얠뺏???鰲????饔낅떽??????");
        }
        if (actions.isEmpty()) {
            actions.add("?耀붾굝梨루땟??????????????? ???β뼯援?癒れ땡??????? ????嚥싲갭큔?댁빆?? ?????ル뒌??亦껋꼦維뽬뜎??????????? ????????????????쇨덫櫻??????????댄뱼????饔낅떽???????????黎앸럽????룸돥??????汝뷴젆?琉????????????????????곸죩.");
        }
        return actions;
    }
    ...
    private List<String> buildComparisonFocus(AnalysisPlan plan, AnalysisDataset dataset) {
        ...
        if (dataset.expenseEntries().isEmpty() && !dataset.comparisonExpenseEntries().isEmpty()) {
            insights.add("?????Β?レ름?????????? ??????????곸궔???????耀붾굝?????????????源낆┸???????癲??됀?????????????????곸궔???????耀붾굝??????????????????????????, ????μ떜媛?걫?繹먃????????????ル뒌?????? ??????????????獄쏅챶留??節뚭퐫???? ??饔낅떽???????????????獄쏅챶留????轅붽틓??????");
        } else {
            insights.add("??????? ??????????곸궔???耀붾굝?????????????? ???????????????곸궔??????" + formatSignedWon(delta) + "(" + formatPercentChange(delta, compareExpense) + ")??????筌?캉??");
        }
        ...
        categoryLabels.stream()
                ...
                .limit(3)
                .forEach(item -> insights.add(item.label() + " ?????곕츥???? " + formatSignedWon(item.delta()) + "."));
        return insights;
    }
    ...
    private String buildTopPaymentMethodInsight(List<PaymentSummaryPayload> expensePayments) {
        if (expensePayments.isEmpty()) {
            return "?耀붾굝?????????????? ??汝뷴젆?琉??????黎앸럽??????????????? ??????源낆┰?????????곸죩.";
        }
        PaymentSummaryPayload top = expensePayments.get(0);
        return "?耀붾굝?????????????? ?????ル뒌?????耀붾굝??????????????汝뷴젆?琉??????黎앸럽?????? " + top.paymentMethodName() + "????? ?????룸챷援???" + formatWon(top.totalAmount()) + "(" + top.entryCount() + "????????筌?캉??";
    }
    ...
    private String formatPercentChange(BigDecimal delta, BigDecimal base) {
        ...
        if (safeBase.compareTo(BigDecimal.ZERO) == 0) {
            return nullToZero(delta).compareTo(BigDecimal.ZERO) == 0 ? "0.00%" : "???????????????곸궔??0????轅붽틓??筌뚮랭沅?????????傭?끆???????????????;
        }
