...
public class LedgerAiAnalysisService {
    ...
    public LedgerAiAnalysisResponse analyze(Long userId, LedgerAiAnalysisRequest request) {
        ...
        try {
            ...
            failedHistory.setSummary("AI ????⑥ル츧癲??????嫄?????????⑤슣??);
    ...
    public LedgerAiAnalysisHistoryPageResponse getHistories(
        ...
        if (createdFrom != null && createdTo != null && createdFrom.isAfter(createdTo)) {
            throw new BadRequestException("????⑥ル츧癲????????????됰븗???棺堉?뤃???????꿔꺂???影?우Ŀ??? ??????닿틢???????댄뱴???????????????깅즽????????놁졄.");
        }
    ...
    public LedgerAiAnalysisHistoryDetailResponse getHistory(Long userId, Long historyId) {
        ...
        LedgerAiAnalysisHistory history = historyRepository.findByIdAndOwnerId(historyId, userId)
                .orElseThrow(() -> new NotFoundException("AI ????⑥ル츧癲????????????됰븗???饔낅떽???????????????깅즽????????놁졄."));
    ...
    private DateRange validateCustomRange(LocalDate from, LocalDate to, String label) {
        if (from == null || to == null) {
            throw new BadRequestException(label + "????????꿔꺂???影?우Ŀ??????곗뵯????????닿틢????μ떜媛?걫????????諛몃마????꿔꺂??????");
        }
        if (from.isAfter(to)) {
            throw new BadRequestException(label + "?????꿔꺂???影?우Ŀ??? ??????닿틢???????댄뱴???????????????깅즽????????놁졄.");
        }
        long days = ChronoUnit.DAYS.between(from, to) + 1;
        if (days > MAX_CUSTOM_RANGE_DAYS) {
            throw new BadRequestException(label + "?? ?饔낅떽?????壤? 1????熬곥끇????ヂ??????源녾텛???????影?력?????????????????놁졄.");
        }
    ...
    private LedgerAiAnalysisReportResponse buildFallbackReport(AnalysisPlan plan, AnalysisDataset dataset) {
        ...
        return new LedgerAiAnalysisReportResponse(
                keySummary,
                fullReport,
                "????影?력???????????댁삩??????癲ル슢???뮻???饔낅떽????????????? " + formatWon(averageExpense) + "????? ??" + expenseCount + "?饔낅챷維??????饔낅떽????????????깅즿????????????????????鶯ㅺ동?????????????????놁졄.",
                notableSpending,
                regularSpending,
                abnormalSpending,
                topPaymentMethod,
    ...
    private String buildKeySummary(AnalysisPlan plan, AnalysisDataset dataset, BigDecimal totalExpense, BigDecimal averageExpense, long expenseCount, List<String> comparisonFocus) {
        if (expenseCount == 0) {
            return "????影?력???????????댁삩???????饔낅떽????????????깅즿?????????깅즿?????????????????⑥ル츧癲??????ш끽維귞댆???????????";
        }
        String range = plan.primaryRange().from() + " ~ " + plan.primaryRange().to();
        if (plan.mode() == LedgerAiAnalysisMode.COMPARISON && dataset.comparisonOverview() != null) {
            ...
            return range + " ?饔낅떽????????????? " + formatWon(totalExpense) + "???????????????????댁삩??????" + formatSignedWon(delta)
                    + "(" + formatPercentChange(delta, compareExpense) + ")??????戮?Ĳ?? "
                    + (comparisonFocus.isEmpty() ? "??????????????? ?????댄뱼?? ????⑥ル츧癲????????諛몃마????꿔꺂??????" : comparisonFocus.get(0));
        }
        return range + " ?饔낅떽????????????? ??" + formatWon(totalExpense) + ", ???癲ル슢???뮻??" + formatWon(averageExpense)
                + "??????戮?Ĳ?? " + expenseCount + "?饔낅챷維??????饔낅떽???????????????????????????????????????????????먃?????ル늉??沃섃뫖荑?????????? ??轅붽틓???????????????????놁졄.";
    }
    ...
    private String buildFullReport(
        ...
        report.append("????⑥ル츧癲???????????댁삩??? ").append(plan.primaryRange().from()).append("?????뼿???").append(plan.primaryRange().to()).append("????롮쾸?椰????????戮?Ĳ?? ")
                .append("???饔낅떽????????????? ").append(formatWon(totalExpense)).append(", ???癲ル슢???뮻???饔낅떽????????????? ").append(formatWon(averageExpense))
                .append("??????饔낅떽????????????깅즿??? ").append(expenseCount).append("?饔낅챷維???????????놁졄. ");
        if (plan.mode() == LedgerAiAnalysisMode.COMPARISON && dataset.comparisonOverview() != null) {
            report.append("??????????????댁삩??").append(plan.comparisonRange().from()).append("?????뼿???").append(plan.comparisonRange().to()).append("????롮쾸?椰?????饔낅떽????????????? ")
                    .append(formatWon(dataset.comparisonOverview().expense())).append("?? ?????????⑥ル츧癲????????? ?饔낅떽???????饔낅떽??影?곗몡???紐?怨뚰뇞?됀??壤굿??????饔낅떽??影?곗몡???紐???????⑥ル츧癲??딅?鍮???怨닿퍖????轅붽틓??????????汝뷴젆??녷뉩??읂???棺堉?뤃??????????놁졄. ");
    ...
    private List<String> buildNotableSpending(AnalysisDataset dataset) {
        ...
        dataset.categoryBreakdown().stream()
                .limit(3)
                .forEach(item -> items.add(categoryLabel(item.groupName(), item.detailName()) + "??" + formatWon(item.totalAmount()) + "(" + item.entryCount() + "???????????????????놁졄."));
        dataset.topExpenses().stream()
                .limit(3)
                .forEach(item -> items.add("?????????饔낅떽???????????諛몃마??? " + item.entryDate() + " " + item.title() + " " + formatWon(item.amount()) + "."));
        return items;
    }
    ...
    private List<String> buildRegularSpending(AnalysisDataset dataset) {
        return dataset.categoryBreakdown().stream()
                ...
                .limit(3)
                .map(item -> categoryLabel(item.groupName(), item.detailName()) + "?? " + item.entryCount() + "?饔낅챷維????????????썹땟戮녹????????????????? ????遺븍き??몄Ŀ?잙㉡彛???????놁졄.")
                .toList();
    }
    ...
    private List<String> buildAbnormalSpending(AnalysisDataset dataset, BigDecimal averageExpense) {
        ...
        dataset.topExpenses().stream()
                ...
                .limit(3)
                .forEach(item -> items.add(item.entryDate() + "??" + item.title() + "(" + formatWon(item.amount()) + ")?? ???癲ル슢???뮻???饔낅떽??????????????????????????饔낅떽????????????????????놁졄."));
        if (items.isEmpty() && !dataset.topExpenses().isEmpty()) {
            ExpenseEntryPayload top = dataset.topExpenses().get(0);
            items.add("????ル늉??????????獒뺣폍???饔낅떽????????????? " + top.entryDate() + " " + top.title() + " " + formatWon(top.amount()) + "??????戮?Ĳ??");
        }
        return items;
    }
    ...
    private List<String> buildSubscriptionInsights(List<RecurringExpenseCandidatePayload> recurringCandidates) {
        List<String> subscriptions = recurringCandidates.stream()
                ...
                .limit(5)
                .map(item -> item.title() + "?? " + item.occurrenceCount() + "??????썹땟戮녹??????꿔꺂???????????影?쀫븸???饔낅떽???????????諛몃마????????戮?Ĳ??")
                .toList();
        return subscriptions.isEmpty() ? List.of("?饔낅떽???嶺뚮슢梨뜹ㅇ????????影?쀫븸??????썹땟戮녹?????饔낅떽????????????? ??轅붽틓?????????? ?????繹먮굝???????") : subscriptions;
    }
    ...
    private List<String> buildFixedExpenseInsights(List<RecurringExpenseCandidatePayload> recurringCandidates) {
        if (recurringCandidates.isEmpty()) {
            return List.of("??????ㅻ쿋?????꿔꺂??씙?猷곗뫀?? ????⑥ル츧癲??딅?鍮? ??棺堉?뤃??????汝뷴젆???????????2???????傭?????썹땟戮녹???????????????????????諛몃마??????轅붽틓?????????? ?????繹먮굝???????");
        }
        return recurringCandidates.stream()
                .limit(5)
                .map(item -> item.title() + "?? " + item.firstDate() + "?????뼿???" + item.lastDate() + "????롮쾸?椰?? " + item.occurrenceCount()
                        + "??????썹땟戮녹?????????????猷몄구????" + formatWon(item.totalAmount()) + "??????戮?Ĳ??")
                .toList();
    }
    ...
    private List<String> buildImprovementActions(AnalysisPlan plan, AnalysisDataset dataset, List<RecurringExpenseCandidatePayload> recurringCandidates, List<String> comparisonFocus) {
        ...
        dataset.categoryBreakdown().stream().findFirst()
                .ifPresent(item -> actions.add(categoryLabel(item.groupName(), item.detailName()) + " ?饔낅떽??????????????????????? ?????????????ル늉??沃섃뫖荑??????傭?끆???????떈????ル늉?? ????ル늉???????????낆Ĳ??"));
        if (!recurringCandidates.isEmpty()) {
            actions.add("????썹땟戮녹?????饔낅떽???????????諛몃마???????嚥싲갭큔?源??????????????띻콞????諛몃마??維◈?癰궽욱룑????源녾텛? ??轅붽틓???????????濡ろ뜐??? ????⑥ル럯?????????????? ??? ???????棺堉?뤃?????????⑹름????ㅿ폍?????怨쀫뮡???????棺堉?뤃??????影?력???轅붽틓?????");
        }
        if (plan.mode() == LedgerAiAnalysisMode.COMPARISON && !comparisonFocus.isEmpty()) {
            actions.add("??????????????댁삩??????????饔낅떽??影?곗몡???紐???????⑥ル츧癲??딅?鍮???怨닿퍖?????嚥싲갭큔????????????댁삩??????????????됰슣類???????읐?癰????筌?痢??????곸궔?????녾컯???觀萸???轅붽틓?????");
        }
        if (actions.isEmpty()) {
            actions.add("?饔낅챷維??????????????? ??嚥▲굧?먪춯??????? ????濚밸Ŧ援앭젆? ????ル늉??沃섃뫖荑??????????? ???????????????ㅼ굣塋?????????살퓢????轅붽틓??????????汝뷴젆??녷뉩??읂???棺堉?뤃???????????????????놁졄.");
        }
        return actions;
    }
    ...
    private List<String> buildComparisonFocus(AnalysisPlan plan, AnalysisDataset dataset) {
        ...
        if (dataset.expenseEntries().isEmpty() && !dataset.comparisonExpenseEntries().isEmpty()) {
            insights.add("????⑥ル츧癲????????? ?????????댁삩???????饔낅떽????????????깅즿???????力?肉????????????????댁삩???????饔낅떽???????????????????????얠?, ???嚥싲갭큔?源???????????ル늉?????? ??????????????諛몃마??ｌ꽔???? ??轅붽틓??????????????諛몃마????꿔꺂??????");
        } else {
            insights.add("??????? ?????????댁삩???饔낅떽????????????? ??????????????댁삩??????" + formatSignedWon(delta) + "(" + formatPercentChange(delta, compareExpense) + ")??????戮?Ĳ??");
        }
        ...
        categoryLabels.stream()
                ...
                .limit(3)
                .forEach(item -> insights.add(item.label() + " ????쇰뮛???? " + formatSignedWon(item.delta()) + "."));
        return insights;
    }
    ...
    private String buildTopPaymentMethodInsight(List<PaymentSummaryPayload> expensePayments) {
        if (expensePayments.isEmpty()) {
            return "?饔낅떽????????????? ??棺堉?뤃??????汝뷴젆?????????????? ??????깅즽????????놁졄.";
        }
        PaymentSummaryPayload top = expensePayments.get(0);
        return "?饔낅떽????????????? ????ル늉?????饔낅떽?????????????棺堉?뤃??????汝뷴젆????? " + top.paymentMethodName() + "????? ????猷몄구???" + formatWon(top.totalAmount()) + "(" + top.entryCount() + "????????戮?Ĳ??";
    }
    ...
    private String formatPercentChange(BigDecimal delta, BigDecimal base) {
        ...
        if (safeBase.compareTo(BigDecimal.ZERO) == 0) {
            return nullToZero(delta).compareTo(BigDecimal.ZERO) == 0 ? "0.00%" : "??????????????댁삩??0????꿔꺂??琉뷩궘?????????鶯ㅺ동??????????????;
        }
