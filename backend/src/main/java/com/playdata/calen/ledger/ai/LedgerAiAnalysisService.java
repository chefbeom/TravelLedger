...
public class LedgerAiAnalysisService {
    ...
    public LedgerAiAnalysisHistoryPageResponse getHistories(
        ...
        if (createdFrom != null && createdTo != null && createdFrom.isAfter(createdTo)) {
            throw new BadRequestException("??????????已???????????????⑤벤????癲됱빖???嶺???????耀붾굝?????傭?끆????椰??? ????????源낅펰????????熬곣몿????????????????濚밸Ŧ援???????????⑤챷竊?");
        }
    ...
    public LedgerAiAnalysisHistoryDetailResponse getHistory(Long userId, Long historyId) {
        ...
        LedgerAiAnalysisHistory history = historyRepository.findByIdAndOwnerId(historyId, userId)
                .orElseThrow(() -> new NotFoundException("AI ??????????已???????????????⑤벤???????븐뼐?????????????????濚밸Ŧ援???????????⑤챷竊?"));
    ...
    private DateRange validateCustomRange(LocalDate from, LocalDate to, String label) {
        if (from == null || to == null) {
            throw new BadRequestException(label + "????????耀붾굝?????傭?끆????椰????????Β?ル윲??????????源낅펰???????濡?씀?濾?????????袁⑸즴筌?씛彛????耀붾굝????????");
        }
        if (from.isAfter(to)) {
            throw new BadRequestException(label + "?????耀붾굝?????傭?끆????椰??? ????????源낅펰????????熬곣몿????????????????濚밸Ŧ援???????????⑤챷竊?");
        }
        long days = ChronoUnit.DAYS.between(from, to) + 1;
        if (days > MAX_CUSTOM_RANGE_DAYS) {
            throw new BadRequestException(label + "?? ????븐뼐????????? 1??????????????亦낃콛???????嚥싲갭큔?????????傭?끆?????????????????????????⑤챷竊?");
        }
    ...
    private LedgerAiAnalysisReportResponse buildFallbackReport(AnalysisPlan plan, AnalysisDataset dataset) {
        ...
        return new LedgerAiAnalysisReportResponse(
                keySummary,
                fullReport,
                "????傭?끆???????????????????⑤㈇????????饔낅떽??????믩베????????븐뼐??????????????? " + formatWon(averageExpense) + "????? ??" + expenseCount + "????븐뼐?곭춯?竊??????????븐뼐??????????????濚밸Ŧ援????????????????????????????????????????????⑤챷竊?",
                notableSpending,
                regularSpending,
                abnormalSpending,
                topPaymentMethod,
    ...
    private String buildKeySummary(AnalysisPlan plan, AnalysisDataset dataset, BigDecimal totalExpense, BigDecimal averageExpense, long expenseCount, List<String> comparisonFocus) {
        if (expenseCount == 0) {
            return "????傭?끆???????????????????⑤㈇???????????븐뼐??????????????濚밸Ŧ援??????????濚밸Ŧ援????????????????????????已??????????諛몃마?잙???????????????";
        }
        String range = plan.primaryRange().from() + " ~ " + plan.primaryRange().to();
        if (plan.mode() == LedgerAiAnalysisMode.COMPARISON && dataset.comparisonOverview() != null) {
            ...
            return range + " ????븐뼐??????????????? " + formatWon(totalExpense) + "?????????????????????⑤㈇???????" + formatSignedWon(delta)
                    + "(" + formatPercentChange(delta, compareExpense) + ")??????癲?筌?? "
                    + (comparisonFocus.isEmpty() ? "??????????????? ??????熬곣몿??? ??????????已??????????袁⑸즴筌?씛彛????耀붾굝????????" : comparisonFocus.get(0));
        }
        return range + " ????븐뼐??????????????? ??" + formatWon(totalExpense) + ", ????饔낅떽??????믩베????" + formatWon(averageExpense)
                + "??????癲?筌?? " + expenseCount + "????븐뼐?곭춯?竊??????????븐뼐??????????????????????????????????????????????????誘⑦떟?????????ル??????붺몭??잞쭜???????????? ???遺얘턁???????????????????????⑤챷竊?";
    }
    ...
    private String buildFullReport(
        ...
        report.append("??????????已??????????????⑤㈇???? ").append(plan.primaryRange().from()).append("???????깆궔????").append(plan.primaryRange().to()).append("??????????????????癲?筌?? ")
                .append("??????븐뼐??????????????? ").append(formatWon(totalExpense)).append(", ????饔낅떽??????믩베????????븐뼐??????????????? ").append(formatWon(averageExpense))
                .append("?????????븐뼐??????????????濚밸Ŧ援???? ").append(expenseCount).append("????븐뼐?곭춯?竊??????????????⑤챷竊? ");
        if (plan.mode() == LedgerAiAnalysisMode.COMPARISON && dataset.comparisonOverview() != null) {
            report.append("????????????????⑤㈇???").append(plan.comparisonRange().from()).append("???????깆궔????").append(plan.comparisonRange().to()).append("??????????????????븐뼐??????????????? ")
                    .append(formatWon(dataset.comparisonOverview().expense())).append("?? ???????????????已?????????? ????븐뼐????????????븐뼐????傭?끆?????Β?ｊ콞???癲????쇰뮡?????????????????????븐뼐????傭?끆?????Β?ｊ콞???癲?????????????已????????????삳섕??????遺얘턁?????????????뀀맩鍮??????룸챶猷??????癲됱빖???嶺????????????⑤챷竊? ");
    ...
    private List<String> buildNotableSpending(AnalysisDataset dataset) {
        ...
        dataset.categoryBreakdown().stream()
                .limit(3)
                .forEach(item -> items.add(categoryLabel(item.groupName(), item.detailName()) + "??" + formatWon(item.totalAmount()) + "(" + item.entryCount() + "?????????????????????⑤챷竊?"));
        dataset.topExpenses().stream()
                .limit(3)
                .forEach(item -> items.add("????????????븐뼐??????????????袁⑸즴筌?씛彛??? " + item.entryDate() + " " + item.title() + " " + formatWon(item.amount()) + "."));
        return items;
    }
    ...
    private List<String> buildRegularSpending(AnalysisDataset dataset) {
        return dataset.categoryBreakdown().stream()
                ...
                .limit(3)
                .map(item -> categoryLabel(item.groupName(), item.detailName()) + "?? " + item.entryCount() + "????븐뼐?곭춯?竊??????????????獄쏅챶留덌┼???????????????????? ??????됰엨????嶺뚮ㅎ?????노짒傭?????????⑤챷竊?")
                .toList();
    }
    ...
    private List<String> buildAbnormalSpending(AnalysisDataset dataset, BigDecimal averageExpense) {
        ...
        dataset.topExpenses().stream()
                ...
                .limit(3)
                .forEach(item -> items.add(item.entryDate() + "??" + item.title() + "(" + formatWon(item.amount()) + ")?? ????饔낅떽??????믩베????????븐뼐???????????????????????????????븐뼐????????????????????????⑤챷竊?"));
        if (items.isEmpty() && !dataset.topExpenses().isEmpty()) {
            ExpenseEntryPayload top = dataset.topExpenses().get(0);
            items.add("???????ル????????????沃섃뫖荑???????븐뼐??????????????? " + top.entryDate() + " " + top.title() + " " + formatWon(top.amount()) + "??????癲?筌??");
        }
        return items;
    }
    ...
    private List<String> buildSubscriptionInsights(List<RecurringExpenseCandidatePayload> recurringCandidates) {
        List<String> subscriptions = recurringCandidates.stream()
                ...
                .limit(5)
                .map(item -> item.title() + "?? " + item.occurrenceCount() + "???????獄쏅챶留덌┼?????????耀붾굝?????????????傭?끆??????レ춸??????븐뼐??????????????袁⑸즴筌?씛彛????????癲?筌??")
                .toList();
        return subscriptions.isEmpty() ? List.of("????븐뼐??????轅붽틓?????獄쎼끇?????????傭?끆??????レ춸???????獄쏅챶留덌┼???????????븐뼐??????????????? ???遺얘턁???????????? ??????μ떜媛?걫????????") : subscriptions;
    }
    ...
    private List<String> buildFixedExpenseInsights(List<RecurringExpenseCandidatePayload> recurringCandidates) {
        if (recurringCandidates.isEmpty()) {
            return List.of("?????????꾨굴?????耀붾굝????????룰퀬?節뉖븶??? ??????????已??????? ??癲됱빖???嶺???????뀀맩鍮?????????????2??????????????獄쏅챶留덌┼???????????????????????????袁⑸즴筌?씛彛???????遺얘턁???????????? ??????μ떜媛?걫????????");
        }
        return recurringCandidates.stream()
                .limit(5)
                .map(item -> item.title() + "?? " + item.firstDate() + "???????깆궔????" + item.lastDate() + "???????????? " + item.occurrenceCount()
                        + "???????獄쏅챶留덌┼??????????????????룸챷援?????" + formatWon(item.totalAmount()) + "??????癲?筌??")
                .toList();
    }
    ...
    private List<String> buildImprovementActions(AnalysisPlan plan, AnalysisDataset dataset, List<RecurringExpenseCandidatePayload> recurringCandidates, List<String> comparisonFocus) {
        ...
        dataset.categoryBreakdown().stream().findFirst()
                .ifPresent(item -> actions.add(categoryLabel(item.groupName(), item.detailName()) + " ????븐뼐????????????????????????? ????????????????ル??????붺몭??잞쭜??????????????????????????ル??? ???????ル????????????????"));
        if (!recurringCandidates.isEmpty()) {
            actions.add("?????獄쏅챶留덌┼???????????븐뼐??????????????袁⑸즴筌?씛彛???????????롮쾸?椰?嚥싲갭怡??????????????????꾨㎜?????袁⑸즴筌?씛彛??癲??됀????⑤슢堉?繹먮굞?????嚥싲갭큔??? ???遺얘턁??????????????棺堉?뤃???? ??????????ъ몥?????????????? ??? ???????癲됱빖???嶺??????????獄쏅챷???????源낆┰??????????????????癲됱빖???嶺??????傭?끆??????????遺얘턁???????");
        }
        if (plan.mode() == LedgerAiAnalysisMode.COMPARISON && !comparisonFocus.isEmpty()) {
            actions.add("????????????????⑤㈇??????????????븐뼐????傭?끆?????Β?ｊ콞???癲?????????????已????????????삳섕??????????롮쾸?椰??????????????⑤㈇?????????????????⑥ル츧癲?????????믨틵?????????癲????????ㅳ늾????????醫딇떍???轝꿸섣??????遺얘턁???????");
        }
        if (actions.isEmpty()) {
            actions.add("????븐뼐?곭춯?竊???????????????? ???汝뷴젆?琉??誘↔덱???????? ?????關?쒎첎?嫄??怨룻돫?? ???????ル??????붺몭??잞쭜????????????? ??????????????????ㅻ쑋雍???????????袁④뎬?????遺얘턁?????????????뀀맩鍮??????룸챶猷??????癲됱빖???嶺?????????????????????⑤챷竊?");
        }
        return actions;
    }
    ...
    private List<String> buildComparisonFocus(AnalysisPlan plan, AnalysisDataset dataset) {
        ...
        if (dataset.expenseEntries().isEmpty() && !dataset.comparisonExpenseEntries().isEmpty()) {
            insights.add("??????????已?????????? ???????????⑤㈇???????????븐뼐??????????????濚밸Ŧ援????????????????????????????????⑤㈇???????????븐뼐???????????????????????????, ???????롮쾸?椰?嚥싲갭怡???????????????ル??????? ???????????????袁⑸즴筌?씛彛??壤굿??留???? ???遺얘턁?????????????????袁⑸즴筌?씛彛????耀붾굝????????");
        } else {
            insights.add("??????? ???????????⑤㈇???????븐뼐??????????????? ????????????????⑤㈇???????" + formatSignedWon(delta) + "(" + formatPercentChange(delta, compareExpense) + ")??????癲?筌??");
        }
        ...
        categoryLabels.stream()
                ...
                .limit(3)
                .forEach(item -> insights.add(item.label() + " ??????⑤벡瑜???? " + formatSignedWon(item.delta()) + "."));
        return insights;
    }
    ...
    private String buildTopPaymentMethodInsight(List<PaymentSummaryPayload> expensePayments) {
        if (expensePayments.isEmpty()) {
            return "????븐뼐??????????????? ??癲됱빖???嶺???????뀀맩鍮???????????????? ??????濚밸Ŧ援???????????⑤챷竊?";
        }
        PaymentSummaryPayload top = expensePayments.get(0);
        return "????븐뼐??????????????? ???????ル?????????븐뼐???????????????癲됱빖???嶺???????뀀맩鍮??????? " + top.paymentMethodName() + "????? ??????룸챷援????" + formatWon(top.totalAmount()) + "(" + top.entryCount() + "????????癲?筌??";
    }
