...
| Analysis service | `LedgerAiAnalysisService` builds owner-scoped, minimized provider payloads, stores completed or failed history, redacts provider failure details, records metrics, accepts a bounded `clientRequestId`, serializes same-JVM in-flight duplicate requests, and reuses recent duplicate completed analysis results. |
...
8. Duplicate suppression remains provider/model/range/clientRequestId aware, `clientRequestId` stays out of provider payloads, and same-JVM in-flight duplicate requests are serialized before provider calls.
