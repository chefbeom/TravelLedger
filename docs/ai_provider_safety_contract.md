...
| Shared response validation | `LedgerAiRemoteResponseValidator.requireUsable(...)` rejects null, failed, empty, secret-like content, authorization-header echoes, secret-bearing URLs/presigned URLs, oversized text fields or lists, malformed list items, prompt-injection echo, and ledger-mutation-claim responses. |
...
| AI-PROV-08B | Provider list fields must contain non-blank strings only; malformed arrays fail closed before history storage or UI exposure. | `LedgerAiRemoteResponseValidatorTest.rejectsBlankProviderListItem`. |
...
6. Prompt-injection echoes, secret-like output, authorization-header echoes, secret-bearing URLs/presigned URLs, oversized text fields/lists, malformed list items, and mutation claims are rejected.
