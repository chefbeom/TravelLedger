...
| --- | --- | --- | --- |
| Account data export | User can download their own ledger, profile-adjacent metadata, and safe drive/travel/family manifests. | Current password-protected archive with ledger CSV, export metadata, and manifest-only media/file listings. | Owner scope, CSRF, secondary PIN, secret exclusion, and safe manifests remain mandatory. |
...
| Completion response | The download response and archive metadata must not expose operational secrets or storage internals. |
...
| `metadata/export-metadata.json` | Export timestamp, owner identity, requested date range, included files, and safe counts. Its scope is `ledger_csv_metadata_and_file_manifests`. | Contains counts and descriptors, not operational secrets. |
...
| Export result | Download responses and archive metadata must not include file names beyond the attachment name, archive contents, secondary PIN values, tokens, public links, presigned URLs, storage paths, prompts, provider responses, raw GPS, or owner identity fields. |
...
The `data-portability-contract` CI job must pass before promoting changes that affect privacy controls, data export, ledger CSV generation, media manifests, storage links, secondary PIN handling, export responses, or future import/export jobs.
...
| Archive, export response, or metadata includes operational secrets, storage paths, public tokens, presigned URLs, raw GPS, AI prompts, provider responses, file names, archive contents, or secondary PIN values. | Leaks infrastructure and sensitive derived data. |
...
- Export metadata contains no secrets, signed URLs, presigned URLs, public tokens, raw GPS, prompts, provider responses, file names beyond the attachment name, archive contents, secondary PIN values, or storage internals.
