...
| `manifest/drive-items.json` | Safe CalenDrive item manifest without object storage paths, public URLs, presigned URLs, or raw share credentials. |
| `manifest/travel-media.json` | Safe travel media manifest that exposes `hasGpsMetadata` and timestamps but not raw latitude/longitude coordinates. |
| `manifest/family-media.json` | Safe family media manifest without storage paths or temporary access URLs. |
...
`scripts/verify-privacy-control-contract.ps1` keeps this document synchronized with `docs/privacy_control_panel.md`, `PrivacyController`, `PrivacyManagementService`, `DataPortabilityExportService`, privacy tests, the security baseline, and the GitHub Actions `privacy-control-contract` job.
