...
| Upload validation | Drive/travel/family/support/OCR upload paths have different risk profiles. | OCR, Drive, travel, family, and support upload validation anchors are now mapped in `docs/file_upload_security_contract.md` and verified by `scripts/verify-file-upload-security-contract.ps1`; `docs/media_processing_queue_contract.md` now separates the original upload lane, thumbnail backfill/reprocessing lane, and future video/transcode lane. Next define remaining per-feature max sizes and shared policy helpers. | CI gates reject drift in upload validation, presigned object-key scope, fake image handling, thumbnail fail-closed evidence, and media queue/lane ownership. |
...
| PWA/mobile capture | OCR, travel photos, and family album are mobile-heavy. | PWA manifest, production service-worker baseline, and camera capture hints are in place; next build an explicit IndexedDB draft tray for receipt OCR, travel photo uploads, and family album uploads with manual retry/delete, logout cleanup, and backend revalidation. | Production frontend is installable, API/private data is excluded from service-worker caching, and offline drafts never cache API data, tokens, presigned URLs, OCR/AI results, or trusted upload decisions. |
| Drive file versioning | Moves CalenDrive toward a safe family drive with recoverable versions, expiring links, download audit logs, and VIEW/DOWNLOAD/EDIT permissions. | Version table/API/frontend drawer are in place; public-share contract covers expiring links, raw-token-free download audit logs, and permission enforcement; next build owner-facing access-log/share-permission UI and replacement-upload version `N+1`. | Users can inspect/restore own versions, public links expire/revoke/fail closed, download attempts are owner-auditable, and VIEW shares cannot download or save. |
| Family budget/shared goals | Makes Household distinct with family budgets, shared savings goals, travel savings goals, and member spending ratio insights. | Owner-scoped household goal APIs are in place; next wire Household goal/budget cards, then add explicit membership/grant rows, contribution records, and aggregate-only member spending ratio cards. | Users can track shared goals and travel funds with visible scope labels, no cross-user ledger leakage, no hidden ledger mutation, and aggregate member ratios only after explicit sharing. |
| Data portability | Builds trust for personal data platform. | Backend data export returns a secondary-PIN-protected ledger CSV plus safe drive/travel/family media manifests and returns the protected archive directly; `ProfileWorkspace.vue` exposes the privacy export action with date range, secondary-PIN dialog, manifest-only explanation, live status messages, and stable E2E anchors. | Authenticated user can download own archive without exposing secrets, storage paths, raw coordinates, prompts, provider responses, or other users data; future binary export must remain async, bounded, encrypted, and expiring. |
...
| `docs/pwa_mobile_capture.md` | PWA installability, service-worker cache policy, mobile camera capture anchors, and offline IndexedDB draft queue contract for receipt OCR, travel photos, and family album uploads. |
...
| `docs/drive_file_versioning.md` | CalenDrive safe family-drive contract for file versions, owner-scoped restore, expiring links, download audit logs, VIEW/DOWNLOAD/EDIT permissions, and replacement-upload backlog. |
...
| `docs/household_budget_goals.md` | Household family budget, shared savings goal, travel savings goal, member spending ratio, owner/member scope, contribution, export, and mutation safety contract. |
...
| `scripts/verify-household-budget-goals-contract.ps1` | Fails CI if household budget/shared-goal contracts lose owner/member scope, explicit mutation boundaries, export safety, implementation anchors, or release-gate wiring. |
| `scripts/verify-drive-file-versioning-contract.ps1` | Fails CI if CalenDrive file versioning loses owner-scoped backend anchors, frontend drawer/restore anchors, version metadata safety, public-share linkage, roadmap coverage, or release-gate wiring. |
...
| `scripts/verify-pwa-mobile-baseline.ps1` | Fails CI if PWA manifest/app-shell privacy, production-only service worker registration, WCAG/mobile checklist anchors, camera capture hints, or offline draft guardrails drift. |
