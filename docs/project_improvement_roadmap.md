...
| Notification center | Connects disconnected events. | Backend notification APIs now receive AI analysis, OCR failure, scheduled-backup failure, and shared-file events; next wire budget/travel producers and frontend badge. | Users can list/read own notifications and receive AI/OCR/backup/share event records with bounded metadata. |
