...
| Notification center | Connects disconnected events. | Backend notification APIs now receive AI analysis, OCR failure, scheduled-backup failure, privacy cleanup, and shared-file events; frontend route rendering and the topbar unread badge are wired; next wire budget/travel producers. | Users can list/read own notifications, see an owner-scoped unread badge, and receive AI/OCR/backup/privacy/share event records with bounded metadata. |
