...
| Admin/share authorization tests | Admin APIs and public links are high-impact. | Drive public download link tests now cover invalid/revoked access logging and raw-token-free log storage; next add broader admin mutation CSRF/role tests. | Normal users receive `403`; invalid tokens receive controlled errors; public link attempts write owner-scoped fingerprint logs. |
