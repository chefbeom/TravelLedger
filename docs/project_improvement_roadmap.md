...
| Notification center | Connects disconnected events. | Backend notification APIs now receive AI analysis, OCR failure, scheduled-backup failure, privacy cleanup, shared-file, travel-start reminder, and travel budget warning events; frontend route rendering and the topbar unread badge are wired; next wire household producers. | Users can list/read own notifications, see an owner-scoped unread badge, and receive AI/OCR/backup/privacy/share/travel reminder/budget warning event records with bounded metadata. |
