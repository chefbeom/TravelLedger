...
| Notification center | Connects disconnected events. | Backend notification APIs now receive AI analysis, OCR failure, scheduled-backup failure, privacy cleanup, shared-file, and travel-start reminder events; frontend route rendering and the topbar unread badge are wired; next wire budget/household producers. | Users can list/read own notifications, see an owner-scoped unread badge, and receive AI/OCR/backup/privacy/share/travel reminder event records with bounded metadata. |
