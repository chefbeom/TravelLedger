...
| AI safety | Ledger AI sends sensitive spending data to LM Studio or n8n and stores results. | Provider response validation, LM Studio prompt hardening, external workflow metrics, provider payload minimization, and 5-minute duplicate suppression are in place; next add production URL allowlist and regression tests. | Provider failures store failed history; invalid JSON/schema is rejected, provider failure/latency can be alerted, provider-facing entry data is capped/truncated, and quick retries reuse completed results. |
