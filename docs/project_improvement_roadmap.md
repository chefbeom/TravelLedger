...
| Drive file versioning | Makes CalenDrive safer for family use. | Version table, upload-created version rows, and owner-scoped version list API are in place; restore previous version remains next. | Uploading a new file records version 1, and the version API is owner-scoped. |
