...
| CI gates | Manual checks are easy to skip. | Run backend test, frontend build, reusable PowerShell secret scan, secret-scan-contract drift checks, and config sync in GitHub Actions. | Push/PR triggers automated backend, frontend, config, high-risk secret gates, and scanner contract checks. |
