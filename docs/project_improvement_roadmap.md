...
| E2E smoke coverage | Core user flows need browser-level evidence beyond backend unit/integration tests. | Use `docs/e2e_smoke_checklist.md` plus `frontend/e2e/smoke.spec.js` for login, ledger entry, Excel import, OCR confirm-save, drive share, admin backup, travel upload, AI analysis, and notifications. The Playwright skeleton now covers session/route checkpoints, fixture-gated P0 flow placeholders, and a notification-center API/UI smoke path. | P0 smoke evidence is attached before releases that change those flows; automated runs must show passing feature-specific assertions or approved skips. |
...
| Family budget/shared goals | Makes Household distinct with family budgets, shared savings goals, travel savings goals, and member spending ratio insights. | Owner-scoped household goal API and bounded `GOAL_PROGRESS` notifications are in place; next wire Household goal/budget cards, then add explicit membership/grant rows, contribution records, and aggregate-only member spending ratio cards. | Users can track shared goals and travel funds with visible scope labels, no cross-user ledger leakage, no hidden ledger mutation, and aggregate member ratios only after explicit sharing. |
| Notification center | Connects disconnected events into one actionable inbox. | Backend/frontend notification center is wired for budget exceeded, AI analysis completed, backup failed, shared file received, travel starts soon, OCR failed, privacy/export, and household goal progress events; next improve category filters, owner access-log links, and household budget cards. | Users can list/read own notifications, see an owner-scoped unread badge, and act on budget/AI/backup/share/travel/OCR events without leaking prompts, tokens, storage paths, raw OCR data, or private member ledger details. |
...
| `docs/notification_center.md` | Notification API contract, unified inbox categories, safety rules, producer queue, and test backlog for budget exceeded, AI analysis completed, backup failed, shared file received, travel starts soon, and OCR failed events. |
...
| `docs/household_budget_goals.md` | Household family budget, shared savings goal, travel savings goal, member spending ratio, owner/member scope, contribution, export, notification, and mutation safety contract. |
...
| `scripts/verify-notification-center-contract.ps1` | Fails CI if notification center loses owner scope, unified inbox categories, redaction, bounded metadata, relative target links, frontend/API anchors, roadmap coverage, or release-gate wiring. |
