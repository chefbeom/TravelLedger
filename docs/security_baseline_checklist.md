...
| P1 | remember-me logout, token rotation, profile credential-change revocation, admin-deactivation revocation, and cookie attributes are covered | `SecurityConfig`, `AuthController`, `AdminService`, `LedgerEntryUserScopeIntegrationTest`, `ProfileCredentialIntegrationTest`, `AdminDashboardIntegrationTest`, future auth integration tests |
