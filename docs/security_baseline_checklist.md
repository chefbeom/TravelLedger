...
| P0 | public download token fails when token is blank/invalid, item is non-file/trashed/missing storage path, or link is revoked/expired/over limit, and each attempt writes an owner-scoped access log without raw token storage | `DriveDownloadLinkServiceTest.listAccessLogsRejectsNonOwnerBeforeReadingLogs`, `DriveDownloadLinkAccessLogServiceTest` |
