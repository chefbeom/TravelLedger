...
| FILE-01 | Upload size limits must be defined per feature. | Spring multipart max is 1GB; OCR has `LEDGER_OCR_MAX_FILE_SIZE`; travel storage validates some flows. | Define per-feature limits for drive, travel media, family album, support attachment, OCR. | Tests for too-large file rejection per upload endpoint. |
| FILE-02 | Upload content type must be validated server-side and not trusted from filename only. | Travel storage verifies several content types; OCR checks image-like upload path. | Centralize allowed MIME/extension rules per feature. Reject ambiguous files. | Tests for spoofed extension/content type mismatch. |
...
| `/api/file/public-download/*` | public drive download token | Add expiry/revoke/access log tests. |
...
| P1 | presigned upload completion rejects object key outside expected owner/record scope | `TravelMediaStorageService` |
