    ...
    <template v-else-if="!currentUser">
      <section class="auth-shell auth-shell--travel-preview">
        ...
        <div class="auth-travel-panel">
          ...
          <article class="auth-card auth-card--travel-login">
            <div class="auth-travel-login__heading">
              <span>ACCOUNT ACCESS</span>
              <h2>로그인</h2>
            </div>
            <form class="stack-form stack-form--login" @submit.prevent="handleLogin">
            ...
            <aside class="auth-signup-note" aria-label="회원가입 안내">
              <div>
                <strong>{{ registrationOptions.publicRegistrationEnabled ? '처음 방문하셨나요?' : '계정이 필요하신가요?' }}</strong>
                <span v-if="isRegistrationOptionsLoading">가입 방법을 확인하고 있습니다.</span>
                <span v-else-if="registrationOptions.publicRegistrationEnabled">공개 회원가입이 열려 있습니다.</span>
                <span v-else>현재는 초대 링크로만 가입할 수 있습니다.</span>
              </div>
              <button
                v-if="!isRegistrationOptionsLoading && registrationOptions.publicRegistrationEnabled"
                class="button button--ghost"
                type="button"
