...
function openReceiptCameraCapture() {
  receiptCameraInputRef.value?.click()
}
...
function canScrollElementByWheel(element, deltaX, deltaY) {
...
function handleReceiptOcrModalWheel(event) {
  event.stopPropagation()
  const modal = event.currentTarget
  let node = event.target
  while (node && node !== modal) {
    if (canScrollElementByWheel(node, event.deltaX, event.deltaY)) {
      return
    }
    node = node.parentElement
  }
  if (canScrollElementByWheel(modal, event.deltaX, event.deltaY)) {
    return
  }
...
function openReceiptHistoryDetail(history) {
  if (normalizeReceiptHistoryStatus(history?.status) === 'PROCESSING') {
    return
  }
  emit('open-receipt-history-detail', history)
}

function closeReceiptHistoryDetail() {
  emit('close-receipt-history-detail')
}
...
function normalizeReceiptHistoryStatus(status) {
...
function getReceiptHistoryActionLabel(history) {
  return isReceiptHistoryCompleted(history) ? '다시 검수' : '기록 확인'
}
function getReceiptHistoryStatusLabel(status) {
  const normalized = String(status || '').trim().toUpperCase()
  if (normalized === 'COMPLETED') return '완료'
  if (normalized === 'FAILED') return '실패'
  if (normalized === 'CANCELLED') return '취소됨'
  if (normalized === 'PROCESSING') return '처리 중'
  return '확인 필요'
}
...
function formatReceiptOcrTiming(timing) {
  ...
  if (Number.isFinite(Number(timing.ocrMs))) {
    parts.push(`이미지 ${Math.round(Number(timing.ocrMs) / 1000)}초`)
  }
...
const receiptHistoryDetailHistory = computed(() => (
  Array.isArray(props.receiptOcr?.historyItems)
    ? props.receiptOcr.historyItems.find((history) => String(history.id) === receiptHistoryDetailAnalysisId.value) || null
    : null
))
const receiptHistoryDetailItem = computed(() => (
  receiptHistoryDetailAnalysisId.value
    ? receiptReviewItems.value.find((item) => String(item.analysisId) === receiptHistoryDetailAnalysisId.value) || null
    : null
))
const isReceiptHistoryDetailOpen = computed(() => receiptOcrView.value === 'history' && Boolean(receiptHistoryDetailAnalysisId.value))
const receiptVisibleReviewItems = computed(() => {
  if (receiptOcrView.value === 'analyze') return receiptAnalyzeItems.value
  if (receiptOcrView.value === 'history') return receiptHistoryDetailItem.value ? [receiptHistoryDetailItem.value] : []
  return []
})
...
function normalizeAggregateGridPosition(value, fallback = 1, max = 4) {
  const numericValue = Number(value)
  if (!Number.isFinite(numericValue)) return fallback
  return Math.min(max, Math.max(1, Math.round(numericValue)))
}
...
function getAggregateCardGridStyle(card) {
  ...
  return {
    '--aggregate-card-col-span': String(width),
    '--aggregate-card-row-span': String(height),
    '--aggregate-card-grid-column': `${column} / span ${width}`,
    '--aggregate-card-grid-row': `${row} / span ${height}`,
    order,
  }
}
...
function isAggregateGridSpaceFree(occupied, column, row, width, height) {
...
function reserveAggregateGridSpace(occupied, column, row, width, height) {
  for (let y = row; y < row + height; y += 1) {
    for (let x = column; x < column + width; x += 1) {
      occupied.add(`${x}:${y}`)
    }
...
function findAggregateGridSlot(occupied, preferredColumn, preferredRow, width, height) {
  const maxColumn = getAggregateMaxColumnForWidth(width)
  const maxRow = getAggregateMaxRowForHeight(height)
  if (isAggregateGridSpaceFree(occupied, preferredColumn, preferredRow, width, height)) {
    return { column: preferredColumn, row: preferredRow }
  }

  for (let row = 1; row <= maxRow; row += 1) {
    for (let column = 1; column <= maxColumn; column += 1) {
      if (isAggregateGridSpaceFree(occupied, column, row, width, height)) {
        return { column, row }
      }
    }
  }
...
function normalizeAggregateConfigs(configs) {
  ...
  const normalizedConfigs = fallback.map((baseConfig, index) => {
...
function updateAggregateWidgetSize(index, value) {
  const currentConfig = aggregateWidgetDraftConfigs.value[index] ?? {}
  const size = parseAggregateWidgetSize(value, currentConfig)
...
function updateAggregateWidgetLayout(index, patch) {
  aggregateWidgetDraftConfigs.value = normalizeAggregateConfigs(
    aggregateWidgetDraftConfigs.value.map((config, configIndex) => (
...
function serializeAggregateWidgetConfigs(configs) {
  return normalizeAggregateConfigs(configs).slice(0, aggregateWidgetMaxCount).map(({ kind, period, paymentMethodId, amountType, monthlyExpenseTarget, singleExpenseLimit, showIncomeCumulative, showExpenseCumulative, comparePreviousPeriod, chartMaxAmount, textSize, textColor, layoutX, layoutY, layoutW }, configIndex) => {
...
function saveAggregateWidgetConfigs() {
  emit('save-aggregate-widget-configs', serializeAggregateWidgetConfigs(aggregateWidgetDraftConfigs.value))
  isAggregateEditMode.value = false
}
...
function listDateRange(from, to, maxDays = 370) {
  const dates = []
  let cursor = from
  while (/^\d{4}-\d{2}-\d{2}$/.test(cursor) && cursor <= to && dates.length < maxDays) {
    dates.push(cursor)
...
const aggregateChartHoveredPoint = ref(null)
const aggregateChartActivePoint = computed(() => aggregateChartSelectedPoint.value || aggregateChartHoveredPoint.value)
const aggregateChartMaxAmountDraft = ref('')

const aggregateChartSelectedExpenseEntries = computed(() => {
  const point = aggregateChartSelectedPoint.value
  const chart = aggregateChartDetailCard.value?.chart
  if (!point || !chart) return []
  const sourceEntries = point.comparisonScope === 'previous' ? chart.previousEntries : chart.currentEntries
  return (Array.isArray(sourceEntries) ? sourceEntries : [])
    .filter((entry) => (
      entry?.entryDate === point.date
      && entry?.entryType !== 'INCOME'
      && Number(summarizeEntries([entry]).expense || 0) > 0
    ))
    .slice()
    .sort((left, right) => String(left.entryTime || '99:99').localeCompare(String(right.entryTime || '99:99')))
})
...
function chartXForIndex(index, total) {
  ...
  return total <= 1 ? AGGREGATE_CHART_HORIZONTAL_PADDING + (drawableWidth / 2) : AGGREGATE_CHART_HORIZONTAL_PADDING + (drawableWidth * index) / (total - 1)
}
...
function resolveAggregateChartMaxAmount(currentItems, previousItems, options = {}) {
  ...
  const visibleDataMax = Math.max(currentMax, previousMax)
  if (requestedMax > 0) {
    return visibleDataMax > requestedMax
      ? roundAggregateChartAxisMax(visibleDataMax * 1.05)
      : requestedMax
  }

  return roundAggregateChartAxisMax(visibleDataMax * 1.05)
}
...
function buildChartAreaPoints(pointItems) {
  ...
  return [`${first.x.toFixed(1)},${baseline.toFixed(1)}`, ...pointItems.map((item) => `${item.x.toFixed(1)},${item.y.toFixed(1)}`), `${last.x.toFixed(1)},${baseline.toFixed(1)}`].join(' ')
}

function chartTickLabel(date, period) {
  const month = Number(date.slice(5, 7))
  const day = Number(date.slice(8, 10))
  if (period === 'YEAR') return `${month}월`
  if (period === 'QUARTER' || period === 'MONTH') return `${month}/${String(day).padStart(2, '0')}`
  if (period === 'WEEK') return `${day}일`
...
function buildChartDateTicks(dailyItems, period) {
  ...
  if (period === 'YEAR') {
    ...
    dailyItems.forEach((item, index) => {
      ...
      if (!seenMonths.has(monthKey)) { seenMonths.add(monthKey); indexes.push(index) }
    })
  } else if (period === 'QUARTER') {
    for (let index = 0; index < dailyItems.length; index += 14) indexes.push(index)
  } else if (period === 'MONTH') {
    for (let index = 0; index < dailyItems.length; index += 7) indexes.push(index)
  } else if (period === 'WEEK') {
    dailyItems.forEach((_, index) => indexes.push(index))
  } else {
    indexes.push(0)
  }
...
function buildChartAmountTicks(maxAmount) {
  ...
  return [0, 0.25, 0.5, 0.75, 1].map((ratio) => {
    ...
    return { key: `${ratio}-${value}`, label: ratio === 0 ? '0' : formatCompactNumber(value), value, y: Number(y.toFixed(1)), labelY: Number(Math.max(10, y - 2).toFixed(1)) }
  })
}

function visibleAggregateChartPoints(card) {
...
function openAggregateChartDetail(card) {
  ...
  aggregateChartHoveredPoint.value = null
}
...
function applyAggregateChartMaxAmount() {
  ...
  aggregateChartSelectedPoint.value = selectedPoint
    ? visibleAggregateChartPoints(nextCard).find((point) => point.key === selectedPoint.key) || null
    : null
  aggregateChartHoveredPoint.value = null
...
function selectAggregateChartPoint(point) {
  aggregateChartSelectedPoint.value = point || null
  aggregateChartHoveredPoint.value = null
}

function resolveAggregateChartNearestPoint(event, card) {
  const points = visibleAggregateChartPoints(card)
  if (!points.length || !event?.currentTarget) return null
  ...
  return points.reduce((best, point) => (!best || Math.abs(point.x - x) < Math.abs(best.x - x) ? point : best), null)
}

function selectAggregateChartNearestPoint(event, card, pin = false) {
  const nearest = resolveAggregateChartNearestPoint(event, card)
  if (!nearest) return
  if (pin) {
    selectAggregateChartPoint(nearest)
    return
  }
  if (!aggregateChartSelectedPoint.value) {
    aggregateChartHoveredPoint.value = nearest
  }
}

function clearAggregateChartHover() {
  aggregateChartHoveredPoint.value = null
}

function summarizeEntriesByDate(entries, dateItems) {
  ...
  return dateItems.map((date) => {
    ...
    return { date, income: cumulativeIncome, expense: cumulativeExpense }
  })
}
...
function buildAggregateChartLabelItems(card, mode = 'detail') {
  ...
  const addCandidates = (items, seriesClass, amountFormatter, yOffset) => {
    sampleAggregateChartLabelPointItems(items, maxPoints).forEach((point) => {
      if (!isPreview && aggregateChartActivePoint.value?.key === point.key) return
...
function buildMonthlyCumulativeChartData(entries, range, overview, options = {}) {
  ...
  const currentDataMax = aggregateChartDataMax(currentItems, showIncome, showExpense)
  const comparisonItems = options.comparePreviousPeriod || currentDataMax <= 0 ? previousItems : []
  const targetAmount = normalizeAggregateChartMaxAmount(options.chartMaxAmount)
  const scaleMaxAmount = resolveAggregateChartMaxAmount(currentItems, comparisonItems, {
    showIncome,
    showExpense,
    chartMaxAmount: targetAmount,
  })
  const axisMaxAmount = scaleMaxAmount
  const visibleDataMax = Math.max(
    currentDataMax,
  ...
  const previousIncomePointItems = buildChartPointItems(previousItems, 'income', scaleMaxAmount, `${comparisonLabel} 수입`, 'previous')
    .map((point) => ({ ...point, seriesLabel: `${comparisonLabel} 수입`, comparisonLabel, comparisonScope: 'previous' }))
  const previousExpensePointItems = buildChartPointItems(previousItems, 'expense', scaleMaxAmount, `${comparisonLabel} 지출`, 'previous')
  ...
  return {
    ...
    incomePointItems,
    expensePointItems,
    previousIncomePointItems,
    previousExpensePointItems,
    incomePoints: buildChartPoints(incomePointItems),
    expensePoints: buildChartPoints(expensePointItems),
    previousIncomePoints: buildChartPoints(previousIncomePointItems),
    ...
    amountTicks: buildChartAmountTicks(axisMaxAmount),
    maxAmountLabel: formatCompactNumber(axisMaxAmount),
...
function buildAggregateCard(config, index) {
  ...
  if (config.kind === 'MONTHLY_CUMULATIVE_CHART') {
    ...
    const chartPeriodLabel = aggregateWidgetPeriods.find((item) => item.value === config.period)?.label || '이번 달'
    return { id: config.id || ('aggregate-' + (index + 1)), index, config, title: `${chartPeriodLabel} 누적 그래프`, periodLabel: chartPeriodLabel, totalAmount: chart.headlineAmount, overview: chartOverview, chart, goal: null }
  }
  ...
  <div class="field field--full">
      ...
      <select v-model="entryForm.categoryDetailId">
        ...
        >
          <article
            ...
            :data-aggregate-x="card.config.layoutX"
          ...
          >
            ...
            <div v-if="isAggregateEditMode && card.config.kind === 'MONTHLY_CUMULATIVE_CHART'" class="household-aggregate-chart-options">
              <span class="household-aggregate-chart-options__title">그래프 표시</span>
              <label>
                <input type="checkbox" :checked="card.config.showIncomeCumulative" @change="updateAggregateWidget(card.index, 'showIncomeCumulative', $event.target.checked)">
                수입 누적
              </label>
              <label>
                <input type="checkbox" :checked="card.config.showExpenseCumulative" @change="updateAggregateWidget(card.index, 'showExpenseCumulative', $event.target.checked)">
                지출 누적
              </label>
              <label>
              ...
              <label class="field household-aggregate-chart-options__max-field">
                ...
                <small>&#48708;&#50864;&#47732; &#54364;&#49884; &#51473;&#51064; &#45572;&#51201;&#44050; &#44592;&#51456;&#51004;&#47196; &#51088;&#46041; &#44228;&#49328;&#54616;&#44256;, &#49444;&#51221;&#44050;&#48372;&#45796; &#49892;&#51228; &#45936;&#51060;&#53552;&#44032; &#53356;&#47732; &#52629;&#51012; &#51088;&#46041; &#54869;&#51109;&#54633;&#45768;&#45796;.</small>
              </label>
            </div>
            ...
            >
              <span class="household-aggregate-chart-preview-button__hint">그래프를 눌러 상세 보기</span>
              <div class="household-aggregate-chart household-aggregate-chart--preview" :class="{ 'is-empty': !card.chart.hasEntries }">
                <svg :viewBox="`0 0 ${AGGREGATE_CHART_WIDTH} ${AGGREGATE_CHART_HEIGHT}`" preserveAspectRatio="xMidYMid meet" role="img" aria-label="누적 수입과 지출 미리보기 그래프">
                  ...
                  <polyline v-if="card.config.comparePreviousPeriod && card.config.showIncomeCumulative" :points="card.chart.previousIncomePoints" class="household-aggregate-chart__line household-aggregate-chart__line--income household-aggregate-chart__line--previous" />
                  ...
                  <line v-if="card.chart.targetLineY" :x1="AGGREGATE_CHART_HORIZONTAL_PADDING" :x2="AGGREGATE_CHART_AXIS_END_X" :y1="card.chart.targetLineY" :y2="card.chart.targetLineY" class="household-aggregate-chart__target-line" />
              ...
              <div class="household-aggregate-chart__legend household-aggregate-chart__legend--preview">
                <span v-if="card.config.showIncomeCumulative"><i class="income"></i>수입 누적</span>
                <span v-if="card.config.showExpenseCumulative"><i class="expense"></i>지출 누적</span>
      ...
      >
        <section class="household-aggregate-chart-modal__panel">
          ...
          <div class="household-aggregate-chart-modal__summary">
            <span v-if="aggregateChartDetailCard.config.showIncomeCumulative" class="household-aggregate-chart-modal__metric household-aggregate-chart-modal__metric--income">수입 누적 <strong>{{ formatCurrency(aggregateChartDetailCard.chart.incomeTotal) }}</strong></span>
            ...
            <span class="household-aggregate-chart-modal__metric household-aggregate-chart-modal__metric--previous">지난달 사용금액 <strong>{{ formatCurrency(aggregateChartDetailCard.chart.previousExpenseTotal) }}</strong></span>
          </div>

          <div class="household-aggregate-chart-modal__controls">
            <label class="field household-aggregate-chart-modal__max-field">
              <span class="field__label">&#51060;&#48264;&#45804; &#47785;&#54364; &#44552;&#50529;</span>
          ...
          <div class="household-aggregate-chart-modal__canvas">
            <svg :viewBox="`0 0 ${AGGREGATE_CHART_WIDTH} ${AGGREGATE_CHART_HEIGHT}`" preserveAspectRatio="xMidYMid meet" role="img" aria-label="누적 수입과 지출 상세 그래프" @mousemove="selectAggregateChartNearestPoint($event, aggregateChartDetailCard)" @mouseleave="clearAggregateChartHover" @click="selectAggregateChartNearestPoint($event, aggregateChartDetailCard, true)">
              ...
              <line v-if="aggregateChartDetailCard.chart.targetLineY" :x1="AGGREGATE_CHART_HORIZONTAL_PADDING" :x2="AGGREGATE_CHART_AXIS_END_X" :y1="aggregateChartDetailCard.chart.targetLineY" :y2="aggregateChartDetailCard.chart.targetLineY" class="household-aggregate-chart__target-line" />
              ...
              <g v-if="aggregateChartActivePoint" class="household-aggregate-chart__crosshair" aria-hidden="true">
                <line :x1="aggregateChartActivePoint.x" :x2="aggregateChartActivePoint.x" :y1="AGGREGATE_CHART_TOP_PADDING" :y2="AGGREGATE_CHART_BASELINE_Y" />
                <line :x1="AGGREGATE_CHART_HORIZONTAL_PADDING" :x2="AGGREGATE_CHART_AXIS_END_X" :y1="aggregateChartActivePoint.y" :y2="aggregateChartActivePoint.y" />
                <circle :cx="aggregateChartActivePoint.x" :cy="aggregateChartActivePoint.y" r="7" />
                <rect :x="aggregateChartCrosshairLabel(aggregateChartActivePoint).backgroundX" :y="aggregateChartCrosshairLabel(aggregateChartActivePoint).backgroundY" :width="aggregateChartCrosshairLabel(aggregateChartActivePoint).backgroundWidth" :height="aggregateChartCrosshairLabel(aggregateChartActivePoint).backgroundHeight" rx="4" class="household-aggregate-chart__crosshair-label-bg" />
                <text :x="aggregateChartCrosshairLabel(aggregateChartActivePoint).x" :y="aggregateChartCrosshairLabel(aggregateChartActivePoint).y" :text-anchor="aggregateChartCrosshairLabel(aggregateChartActivePoint).anchor" dominant-baseline="middle">{{ aggregateChartCrosshairLabel(aggregateChartActivePoint).text }}</text>
              </g>
            </svg>
          </div>

          <div class="household-aggregate-chart__legend household-aggregate-chart-modal__legend">
            <span v-if="aggregateChartDetailCard.config.showIncomeCumulative"><i class="income"></i>수입 누적</span>
            <span v-if="aggregateChartDetailCard.config.showExpenseCumulative"><i class="expense"></i>지출 누적</span>
            <span v-if="aggregateChartDetailCard.config.comparePreviousPeriod"><i class="previous"></i>지난 기간 비교 {{ aggregateChartDetailCard.chart.previousRangeLabel }}</span>
          </div>

          <aside v-if="aggregateChartActivePoint" class="household-aggregate-chart-modal__selected">
            <span>{{ aggregateChartActivePoint.date }} · {{ aggregateChartActivePoint.seriesLabel }}</span>
            <strong>{{ formatCurrency(aggregateChartActivePoint.amount) }}</strong>
            <p>마우스가 가리키는 X축 시간과 Y축 금액의 교차점입니다. 해당 시점의 누적 {{ aggregateChartActivePoint.seriesLabel }} 금액은 {{ formatCurrency(aggregateChartActivePoint.amount) }}이고, 직전 날짜 대비 증가분은 {{ formatCurrency(aggregateChartActivePoint.dailyAmount) }}입니다.</p>
          </aside>
          <section v-if="aggregateChartSelectedPoint" class="household-aggregate-chart-modal__transactions">
            <header class="household-aggregate-chart-modal__transactions-header">
              <div>
                <span class="household-aggregate-chart-modal__eyebrow">EXPENSES</span>
                <strong>{{ formatShortDate(aggregateChartSelectedPoint.date) }} · 지출 내역</strong>
              </div>
              <span class="household-aggregate-chart-modal__transactions-count">{{ aggregateChartSelectedExpenseEntries.length }}건</span>
            </header>
            <ul v-if="aggregateChartSelectedExpenseEntries.length" class="household-aggregate-chart-modal__transactions-list">
              <li v-for="(entry, entryIndex) in aggregateChartSelectedExpenseEntries" :key="entry.id || entryIndex">
                <div>
                  <strong>{{ entry.title || '지출' }}</strong>
                  <small>
                    <template v-if="entry.entryTime && entry.entryTime !== '00:00'">{{ formatTime(entry.entryTime) }}</template>
                    <template v-if="entry.categoryGroupName || entry.categoryDetailName">
                      <span v-if="entry.entryTime && entry.entryTime !== '00:00'"> · </span>
                      {{ entry.categoryGroupName || '' }}<template v-if="entry.categoryDetailName"> / {{ entry.categoryDetailName }}</template>
                    </template>
                  </small>
                </div>
                <strong class="household-aggregate-chart-modal__transaction-amount">{{ formatCurrency(Math.abs(Number(entry.amount || 0))) }}</strong>
              </li>
            </ul>
            <p v-else class="household-aggregate-chart-modal__transactions-empty">해당 날짜에 지출 내역이 없습니다.</p>
          </section>
        </section>
      </div>
              ...
              <article
                ...
                :class="[
                  'calendar__day',
                  {
                    ...
                    'calendar__day--today': day.date === todayIso,
                  },
                ]"
                role="button"
                tabindex="0"
    ...
    >
      <section :class="['receipt-ocr-modal__dialog', { 'receipt-ocr-modal__dialog--review-list': receiptOcrView !== 'rules' && receiptVisibleReviewItems.length, 'receipt-ocr-modal__dialog--history-detail': isReceiptHistoryDetailOpen }]" role="dialog" aria-modal="true" aria-labelledby="receipt-ocr-modal-title">
        ...
        <section
          v-else-if="receiptOcrView === 'history'"
          :class="['receipt-ocr-workspace receipt-ocr-workspace--history', { 'receipt-ocr-workspace--history-detail': isReceiptHistoryDetailOpen }]"
        >
          <template v-if="isReceiptHistoryDetailOpen">
            <div class="receipt-ocr-history-detail__header">
              <button type="button" class="button button--ghost" @click="closeReceiptHistoryDetail">
                &lt; 뒤로가기
              </button>
              <div>
                <strong>{{ receiptHistoryDetailItem?.fileName || receiptHistoryDetailHistory?.fileName || '분석 내용' }}</strong>
                <span>
                  {{ getReceiptDocumentLabel(receiptHistoryDetailItem?.documentType || receiptHistoryDetailHistory?.documentType) }}
                  / {{ getReceiptHistoryStatusLabel(receiptHistoryDetailItem?.analysisStatus || receiptHistoryDetailHistory?.status) }}
                </span>
              </div>
            </div>

            <p v-if="receiptOcr?.historyError" class="receipt-ocr-history__error">{{ receiptOcr.historyError }}</p>
            <p v-if="receiptOcr?.isHistoryDetailLoading" class="receipt-ocr-modal__empty">
              분석 내용을 불러오는 중입니다.
            </p>
            <p v-else-if="!receiptHistoryDetailItem" class="receipt-ocr-modal__empty">
              불러온 상세 내용이 없습니다. 목록으로 돌아가 다시 선택해 주세요.
            </p>

            <div v-else class="receipt-ocr-detail-tools">
              <div class="receipt-ocr-detail-tools__copy">
          ...
          <template v-else>
            <section class="receipt-ocr-history" aria-label="이미지 분석 기록">
              <div class="receipt-ocr-history__header">
                <div>
                  <strong>분석 내역</strong>
                  <span>내용을 확인할 분석 요청을 선택하세요.</span>
                </div>
                <button
                  type="button"
        ...
        <div v-if="receiptOcrView !== 'rules' && receiptVisibleReviewItems.length" :class="['receipt-ocr-modal__list', { 'receipt-ocr-modal__list--detail': isReceiptHistoryDetailOpen }]">
          <article
            v-for="item in receiptVisibleReviewItems"
            :key="item.id"
            :class="['receipt-ocr-review-card', `receipt-ocr-review-card--${item.status}`, { 'receipt-ocr-review-card--detail': isReceiptHistoryDetailOpen }]"
          >
            ...
            <div class="receipt-ocr-review-card__body">
              ...
              <div class="receipt-ocr-review-card__analysis">
                ...
                <p v-else-if="item.status === 'analyzing'" class="receipt-ocr-review-card__message">
                  AI가 이미지에서 거래 항목을 추출하고 있습니다.
                </p>
                <p v-else-if="item.status === 'cancelled'" class="receipt-ocr-review-card__message receipt-ocr-review-card__message--error">
                  취소된 이미지 분석 요청입니다. 기록 확인만 가능하고 입력칸 적용은 막혀 있습니다.
                </p>
...
.receipt-ocr-review-entry__guard {
  margin: 0.35rem 0 0;
  color: #f7c56b;
  font-size: 0.82rem;
  font-weight: 700;
