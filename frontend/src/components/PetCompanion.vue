...
const PET_DIALOGUES = {
  yuna: {
    lines: ['오늘은 어떤 기록을 살펴볼까요?', '잠깐 창밖을 보고 있었어요.', '천천히 해도 괜찮아요.', '잘하고 있어요. 조금만 더 해볼까요?', '필요한 기록은 제가 함께 살펴볼게요.', '오늘의 가계부도 깔끔하게 정리해 봐요.', '휴식도 계획의 일부예요.', '다음 일정이 궁금하면 눌러 주세요.', '작은 기록이 모여 큰 도움이 돼요.', '집중하고 계시네요. 멋져요.', '제가 옆에서 조용히 지켜볼게요.', '한 번 더 확인하면 더 정확해져요.', '기록을 확인하면 흐름이 더 잘 보여요.', '무리하지 말고 천천히 이어가요.', '오늘도 좋은 하루가 될 거예요.'],
    pickup: '앗, 조심히 들어 주세요.',
    drop: '고마워요. 여기서 지켜볼게요.',
  ...
  momo: {
    lines: ['야옹, 오늘은 무엇을 정리할까요?', '햇볕 좋은 자리를 찾았어요.', '기록이 늘어나는 건 좋은 일이에요.', '야옹, 오늘 기록을 함께 볼게요.', '잠깐 스트레칭할 시간이에요.', '궁금한 항목이 있으면 눌러 봐요.', '제가 옆에서 조용히 기다릴게요.', '오늘도 차분하게 해내고 있어요.', '작은 변화도 놓치지 않을게요.', '야옹, 잠깐 쉬어가도 괜찮아요.', '새로운 기록 냄새가 나요.', '기록을 확인하면 마음이 편해져요.', '제 꼬리가 살랑살랑 기분 좋아요.', '다음 할 일을 같이 살펴봐요.', '필요하면 저를 불러 주세요.'],
    pickup: '야옹! 살살 들어 주세요.',
  ...
  bomi: {
    lines: ['멍멍, 오늘도 함께해요!', '꼬리를 흔들며 기다리고 있었어요.', '기록을 잘 살펴보고 있어요.', '새 기록도 함께 살펴볼게요.', '멍멍, 잠깐 쉬어도 좋아요.', '오늘 일정도 순조롭게 진행 중이에요.', '제가 옆에서 응원할게요!', '작은 지출도 잘 확인해 봐요.', '다음 할 일을 알려 주세요.', '멍멍, 좋은 냄새가 나는 하루예요.', '천천히 해도 끝까지 같이 갈게요.', '기록이 깔끔하면 기분이 좋아요.', '필요하면 제 이름을 불러 주세요.', '오늘도 정말 잘하고 있어요.', '오늘 기록도 꼼꼼히 지켜볼게요.'],
...
const PETS = [
  ...
  },
  {
    ...
    kind: 'image',
    asset: momoImageUrl,
    description: '기록을 함께 살펴보는 고양이 펫입니다.',
    greeting: '오늘도 차분히 함께해요.',
  ...
  },
  {
    ...
    kind: 'image',
    asset: bomiImageUrl,
    description: '기록을 함께 살펴보는 강아지 펫입니다.',
    greeting: '오늘 기록도 꼼꼼히 살펴볼게요.',
    petLine: '좋아요. 다음 기록도 함께 살펴볼게요.',
  },
]

function isPetAvailable(petId) {
  return String(petId || '') !== 'yuna' || Boolean(props.isAdmin)
}

function fallbackPetId() {
  return PETS.find((pet) => isPetAvailable(pet.id))?.id || 'momo'
}
...
function loadSettings() {
  ...
  try {
    ...
    return {
      ...defaults,
      ...stored,
      selectedPetId: PETS.some((pet) => pet.id === stored?.selectedPetId) && isPetAvailable(stored?.selectedPetId)
        ? stored.selectedPetId
        : defaults.selectedPetId,
...
const availablePets = computed(() => PETS.filter((pet) => isPetAvailable(pet.id)))
const currentPet = computed(() => (
  availablePets.value.find((pet) => pet.id === settings.selectedPetId)
  || availablePets.value[0]
  || PETS.find((pet) => pet.id === 'momo')
))
const currentPetDialogue = computed(() => PET_DIALOGUES[currentPet.value.id] || PET_DIALOGUES.momo)
...
function selectPet(id) {
  if (!PETS.some((pet) => pet.id === id) || !isPetAvailable(id)) {
    return
  }
...
watch(
  [() => props.isAdmin, () => settings.selectedPetId],
  () => {
    if (isPetAvailable(settings.selectedPetId)) {
      return
    }
    settings.selectedPetId = fallbackPetId()
    isQuickOpen.value = false
    persistSettings()
  },
  { immediate: true },
      ...
      <section class="travel-modal__dialog pet-manager-modal__dialog" role="dialog" aria-modal="true" aria-labelledby="pet-manager-title">
        <header class="travel-modal__header">
          <div>
            ...
            <h2 id="pet-manager-title">펫 관리</h2>
          </div>
        ...
        <div class="pet-manager-modal__body">
          ...
          <section class="pet-manager-modal__settings" aria-label="펫 설정">
            ...
            <label class="pet-manager-toggle">
              <input v-model="settings.announceAloud" type="checkbox" :disabled="!settings.enabled" />
              <span>브라우저 음성 안내</span>
            </label>
...
<style scoped>
.pet-companion__dock { position: fixed; right: 18px; bottom: 18px; z-index: 1100; width: var(--pet-size, 100px); background: transparent !important; border: 0 !important; box-shadow: none !important; touch-action: none; user-select: none; }
.pet-companion__dock.is-dragging { cursor: grabbing; }
.pet-companion__avatar { position: relative; display: grid; width: var(--pet-size, 100px); height: calc(var(--pet-size, 100px) + 14px); padding: 0; appearance: none; -webkit-appearance: none; border: 0 !important; background: transparent !important; background-color: transparent !important; background-image: none !important; box-shadow: none !important; cursor: grab; place-items: center; }
.pet-companion__avatar:hover, .pet-companion__avatar:active { border: 0 !important; background: transparent !important; background-color: transparent !important; background-image: none !important; box-shadow: none !important; }
.pet-companion__dock::before, .pet-companion__dock::after, .pet-companion__avatar::before, .pet-companion__avatar::after { content: none !important; display: none !important; }
.pet-companion__avatar img, .pet-companion__sprite { display: block; width: var(--pet-size, 100px); height: var(--pet-size, 100px); border: 0 !important; background-color: transparent !important; box-shadow: none !important; object-fit: contain; filter: drop-shadow(0 5px 9px rgba(0,0,0,.3)); }
...
.pet-companion__animal-sprite { background-color: transparent !important; border: 0 !important; box-shadow: none !important; transform-origin: 50% 75%; will-change: transform; }
