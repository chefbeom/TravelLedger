...
async function request(path, options = {}) {
  ...
  if (!response.ok) {
    let message = '??븐슙???嶺뚳퐣瑗???濡ル츎 繞???쒖굣?節낆쾸? ?꾩룇裕뉑틦???곕????덈펲.'
    let details = null
...
async function uploadPresignedTravelMediaFile(target, file) {
  ...
  if (!response.ok) {
    throw new Error('???逾????놁Ŧ??繞???쒖굣?節낆쾸? ?꾩룇裕뉑틦???곕????덈펲.')
  }
}
...
function uploadPresignedTravelMediaFileWithProgress(target, file) {
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      ...
      reject(new Error('???逾????놁Ŧ??繞???쒖굣?節낆쾸? ?꾩룇裕뉑틦???곕????덈펲.'))
    }
    xhr.onerror = () => reject(new Error('???逾????놁Ŧ??繞???쒖굣?節낆쾸? ?꾩룇裕뉑틦???곕????덈펲.'))
    xhr.onabort = () => reject(new Error('???逾????놁Ŧ??? ???쳛???琉????鍮??'))
    xhr.send(file)
  })
}
...
async function uploadTravelMediaInternal({
  ...
  if (oversizedFile) {
    const error = new Error('??筌???壤?? 15MB ??袁⑤┃ ???逾х춯????놁Ŧ???ル쭑 ?????곕????덈펲.')
    error.status = 400
