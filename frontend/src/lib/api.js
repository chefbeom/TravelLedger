...
const TRAVEL_MEDIA_UPLOAD_CONCURRENCY = 5
...
async function request(path, options = {}) {
  ...
  if (!response.ok) {
    let message = '??Î∏êÏäô???Ô¶´Îö≥?£Áëó???Êø°„É´Ï∏?Áπ????ñÍµ£?ÁØÄ?ÜÏæ∏? ?Íæ©Î£áË£ïÎâë????Í≥?????àÌé≤.'
    let details = null
...
export function analyzeLedgerSpending(payload) {
  return request('/statistics/ai-analysis', {
    method: 'POST',
    body: JSON.stringify(withLedgerAiClientRequestId(payload)),
  })
}
...
export function fetchLedgerExchangeRate(currencyCode, entryDate, entryDateTime = '') {
  return request(buildUrl('/entries/exchange-rate', { currencyCode, entryDate, entryDateTime }).replace(API_BASE, ''))
}
...
export function reflectTravelRecordToLedger(recordId) {
  return request(`/travel/records/${recordId}/ledger-entry`, {
    method: 'POST',
  })
}
...
async function uploadPresignedTravelMediaFile(target, file) {
  ...
  if (!response.ok) {
    throw new Error('?????????Å≈??Áπ????ñÍµ£?ÁØÄ?ÜÏæ∏? ?Íæ©Î£áË£ïÎâë????Í≥?????àÌé≤.')
  }
}
...
function uploadPresignedTravelMediaFileWithProgress(target, file) {
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      ...
      reject(new Error('?????????Å≈??Áπ????ñÍµ£?ÁØÄ?ÜÏæ∏? ?Íæ©Î£áË£ïÎâë????Í≥?????àÌé≤.'))
    }
    xhr.onerror = () => reject(new Error('?????????Å≈??Áπ????ñÍµ£?ÁØÄ?ÜÏæ∏? ?Íæ©Î£áË£ïÎâë????Í≥?????àÌé≤.'))
    xhr.onabort = () => reject(new Error('?????????Å≈??? ???Ï≥???Ôß???????'))
    xhr.send(file)
  })
}
...
async function uploadTravelMediaInternal({
  ...
  if (oversizedFile) {
    const error = new Error('??Á≠???Â£?? 15MB ??Ë¢Å‚ë§??????æ—ÖÏ∂Ø?????Å≈????´Ï≠ë ?????Í≥?????àÌé≤.')
    error.status = 400
...
export function removePrivacyPhotoLocationMetadata() {
  return request('/privacy/photo-location-metadata', {
    method: 'DELETE',
  })
}
