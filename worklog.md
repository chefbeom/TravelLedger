...
### 2026-04-25 - Travel map viewport rendering optimization

- User request: Improve the travel map stutter when dragging or zooming with many clustered image nodes, while ensuring backend load does not increase.
- Request analysis: The safest implementation is frontend-only. The existing travel map overview API continues to load the same data once, and the map component reduces Leaflet DOM marker work by filtering to the current viewport and combining nearby visible markers on the client.
- Actions taken: Checked `codingconvention.md`, reviewed `TravelMyMapClusterPanel.vue`, Leaflet marker rendering, route rendering, and related map marker CSS in `style.css`.
- Implementation: Added padded viewport filtering, client-side grid clustering, selected-marker preservation, and click-to-zoom behavior for client-generated clusters. Added debounced re-rendering on map move/zoom end, disabled heavier Leaflet marker animation, rendered routes through a canvas renderer, and reduced marker shadow/overlay cost while the map is moving. No backend endpoint, request cadence, or API contract was changed.
- Verification: Ran `cmd /c npm run build` in `frontend` successfully. Verified no TypeScript SFC/script or `.ts`/`.tsx` files with `rg -n 'lang="ts"|lang=''ts''' frontend/src`, `Get-ChildItem -Path frontend/src -Recurse -Include *.ts,*.tsx`, and `rg --files frontend/src | rg '\.(ts|tsx)$'`. Ran `git diff --check -- frontend/src/components/TravelMyMapClusterPanel.vue frontend/src/style.css worklog.md` with no whitespace errors. Checked the frontend diff for new backend/API calls and found no added fetch/axios/travel API usage.
- Result: Travel map drag/zoom should render fewer active DOM markers and feel lighter, especially when many photo pins or clusters exist, without adding backend work.

### 2026-04-25 - Travel map cluster interaction fixes

- User request: Fix zoom-level merged travel clusters so their representative image is visible from the earliest photo, one click opens the info flow, Esc closes the enlarged image instead of leaving the map, and clicking empty map space leaves nothing selected.
- Request analysis: These were frontend interaction issues in the client-side map clustering layer and lightbox keyboard handling. The backend API and data loading cadence should remain unchanged.
- Actions taken: Checked `codingconvention.md`, reviewed `TravelMyMapClusterPanel.vue`, `TravelMyMapWorkspace.vue`, `TravelPhotoLightbox.vue`, and the map marker CSS.
- Implementation: Client-generated clusters now keep the real earliest photo/cluster representative instead of replacing it with a synthetic id, and the CSS no longer hides the inline representative thumbnail. Marker clicks now select the real representative item and open the popup immediately on the first click. Empty map clicks emit a clear-selection event while marker and route clicks stop propagation. The lightbox handles Escape in capture phase with default prevention so the image viewer closes before fullscreen map handling.
- Verification: Ran `cmd /c npm run build` in `frontend` successfully. Verified no TypeScript SFC/script or `.ts`/`.tsx` files with `rg -n 'lang="ts"|lang=''ts''' frontend/src`, `Get-ChildItem -Path frontend/src -Recurse -Include *.ts,*.tsx`, and `rg --files frontend/src | rg '\.(ts|tsx)$'`. Ran `git diff --check -- frontend/src/components/TravelMyMapClusterPanel.vue frontend/src/components/TravelMyMapWorkspace.vue frontend/src/components/TravelPhotoLightbox.vue frontend/src/style.css` with no whitespace errors. Checked the frontend diff for new backend/API calls and found none.
- Result: Merged map clusters now show a representative thumbnail, open their info popup/selection on first click, close only the lightbox on Escape, and clear selection only when the user clicks blank map space.

### 2026-04-25 - Travel map merged cluster selection retention

- User request: In zoomed-out merged clusters, selecting a cluster should not pull one photo out of the cluster as a separate marker, and the cluster info window should open on the first click after analyzing the issue.
- Request analysis: The previous selected-marker preservation logic removed the selected real cluster/photo from viewport clustering and appended it as a separate marker. After click, the parent selected id also replaced the pending popup key with the real marker key, while the rendered marker was still the merged viewport cluster key.
- Actions taken: Checked `codingconvention.md`, reviewed `TravelMyMapClusterPanel.vue` clustering, selected-state propagation, and popup key scheduling.
- Implementation: Kept selected items inside viewport clustering so the merged cluster remains a single selected marker. Added popup key fallback so if the pending real marker key is not rendered because it belongs to a merged cluster, the popup opens on the rendered merged cluster marker instead.
- Verification: Ran `cmd /c npm run build` in `frontend` successfully. Verified no TypeScript SFC/script or `.ts`/`.tsx` files with `rg -n 'lang="ts"|lang=''ts''' frontend/src`, `Get-ChildItem -Path frontend/src -Recurse -Include *.ts,*.tsx`, and `rg --files frontend/src | rg '\.(ts|tsx)$'`. Ran `git diff --check -- frontend/src/components/TravelMyMapClusterPanel.vue` with no whitespace errors. Checked the diff for new backend/API calls and found none.
- Result: Selecting a zoomed-out merged cluster now highlights and opens the merged cluster itself, without splitting its representative photo into a separate marker.

### 2026-04-25 - Household calendar sheet and edit focus navigation

- User request: In the household calendar ledger, clicking a calendar date should move to the selected date's transaction sheet below, and clicking edit in that list should focus the transaction edit form.
- Request analysis: Date selection already updated the selected date and attempted a direct scroll, while edit already emitted through the parent into the entry form. The missing piece was making both movements wait for GridStack/layout repaint and focusing a real input after the edit form is populated.
- Actions taken: Checked `codingconvention.md`, reviewed `CalendarWorkspace.vue` date selection, sheet refs, exposed methods, and `HouseholdWorkspace.vue` edit-entry flow.
- Implementation: Added layout-frame-aware panel scrolling for the transaction sheet and entry editor. Date clicks now use the shared scroll helper for reliable movement to the sheet. The exposed `scrollToEntryEditor` now scrolls to the quick entry/edit panel and focuses the first transaction input without changing entry data or backend logic.
- Verification: Ran `cmd /c npm run build` in `frontend` successfully. Verified no TypeScript SFC/script or `.ts`/`.tsx` files with `rg -n 'lang="ts"|lang=''ts''' frontend/src`, `Get-ChildItem -Path frontend/src -Recurse -Include *.ts,*.tsx`, and `rg --files frontend/src | rg '\.(ts|tsx)$'`. Ran `git diff --check -- frontend/src/components/CalendarWorkspace.vue` with no whitespace errors. Checked the diff for new backend/API calls and found none.
- Result: Calendar day selection and sheet edit actions now move the user to the expected part of the calendar ledger flow more reliably.

### 2026-04-25 - Travel map popup flicker fix

- User request: Fix the recurring travel map issue where clicking a cluster briefly opens the info popup and then immediately closes it, and push the completed work.
- Request analysis: The popup was being opened inside the marker click handler before the selected-cluster prop change and client-cluster rerender completed. `renderClusters()` then cleared the marker layer, removing the marker that owned the popup. A possible map background click on the same pointer event could also clear the selection.
- Actions taken: Checked `codingconvention.md`, reviewed the marker click handler, popup scheduling, selection watcher, map background click handler, and current git state.
- Implementation: Changed marker clicks to request a popup and schedule a cluster rerender instead of opening immediately. Popup opening now happens after the final rendered marker exists. Added a popup request sequence so stale open attempts are ignored, and suppresses the matching map background click once so marker clicks do not immediately clear the selection.
- Verification: Ran `cmd /c npm run build` in `frontend` successfully. Verified no TypeScript SFC/script or `.ts`/`.tsx` files with `rg -n 'lang="ts"|lang=''ts''' frontend/src`, `Get-ChildItem -Path frontend/src -Recurse -Include *.ts,*.tsx`, and `rg --files frontend/src | rg '\.(ts|tsx)$'`. Ran `git diff --check -- frontend/src/components/TravelMyMapClusterPanel.vue` with no whitespace errors. Checked the diff for new backend/API calls and found none.
- Result: Travel map cluster popups now open after the cluster layer finishes rerendering, preventing the open-then-disappear flicker while keeping backend load unchanged.

### 2026-04-25 - Household calendar sheet edit focus hardening

- User request: In the household calendar ledger, clicking a month calendar date should scroll to that date's transaction sheet, and clicking edit in the sheet should focus the transaction edit/input area.
- Request analysis: The calendar already had date-to-sheet scrolling and an exposed editor focus method, but the sheet edit event still relied on the parent handler to scroll after filling the form. The safer flow is to keep the parent responsible only for putting the selected entry into edit state, and let the calendar component perform the sheet-to-editor navigation after the DOM updates.
- Actions taken: Checked `codingconvention.md`, reviewed `CalendarWorkspace.vue`, the sheet refs, the exposed focus method, `HouseholdWorkspace.vue` edit-entry binding, and the current dirty worktree to avoid staging unrelated user edits.
- Implementation: The selected date watcher now clears any checked rows in the sheet when moving to another date. Sheet edit buttons now call a dedicated handler that emits `edit-entry`, waits for the parent form fill to render, scrolls to the quick entry/edit panel, and focuses the first editor input. The parent calendar edit binding now fills the form without performing a duplicate scroll.
- Verification: Ran `cmd /c npm run build` in `frontend` successfully. Verified no TypeScript SFC/script or `.ts`/`.tsx` files with `rg -n 'lang="ts"|lang=''ts''' frontend/src` and `Get-ChildItem -Path frontend/src -Recurse -Include *.ts,*.tsx`. Ran `git diff --check -- frontend/src/components/CalendarWorkspace.vue frontend/src/components/HouseholdWorkspace.vue` with no whitespace errors.
- Result: Calendar date selection keeps the transaction sheet state clean, and sheet edit actions now reliably move focus to the populated transaction edit form without changing backend logic.

### 2026-04-25 - Household calendar explicit scroll targets and compact date label

- User request: Remove the quick transaction helper sentence, show the calendar date as a compact year-month-day label based on the selected/current transaction date, and make calendar date clicks and sheet edit buttons reliably move to their intended target panels without breaking existing ledger features.
- Request analysis: The previous movement targeted the inner panel section. In the draggable GridStack calendar layout, the safer target is the outer grid item because that is the element whose absolute layout position represents the visible panel. The entry form title and all transaction input/delete/submit bindings should remain intact.
- Actions taken: Checked `codingconvention.md`, reviewed the current `CalendarWorkspace.vue` diff, scroll helpers, GridStack item markup, quick entry header, and related CSS. Existing unrelated dirty changes were kept out of the staged change.
- Implementation: Added explicit scroll target refs and stable DOM ids for the quick entry editor and transaction sheet grid items. Date clicks now scroll to the transaction sheet target, and sheet edit clicks scroll to the quick entry/edit target before focusing the first editor input. Removed the helper sentence while keeping the quick entry/edit title, and changed the date badge to a compact ISO `YYYY-MM-DD` display with smaller styling.
- Verification: Ran `cmd /c npm run build` in `frontend` successfully. Verified no TypeScript SFC/script or `.ts`/`.tsx` files with `rg -n 'lang="ts"|lang=''ts''' frontend/src` and `Get-ChildItem -Path frontend/src -Recurse -Include *.ts,*.tsx`. Ran `git diff --check -- frontend/src/components/CalendarWorkspace.vue frontend/src/style.css` with no whitespace errors. Checked that submit, delete, edit, date selection, amount, category, payment, and time bindings remain present.
- Result: The quick entry panel no longer shows the helper sentence, the date badge is compact, and both calendar-date-to-sheet and sheet-edit-to-editor navigation use explicit panel targets while preserving existing transaction behavior.

### 2026-04-25 - Household quick entry field row layout

- User request: In the household calendar quick entry form, place date and time on one row, place payment method, major category, and category on one row, and push the completed change.
- Request analysis: The existing quick entry form already had all required bindings. The safest change was to regroup only the rendered fields and add row-specific CSS, leaving the transaction type, amount, date, time, category, payment, memo, submit, edit, and delete logic unchanged.
- Actions taken: Checked `codingconvention.md`, reviewed `CalendarWorkspace.vue` quick entry markup, the parent event bindings in `HouseholdWorkspace.vue`, and the relevant `style.css` editor rules. Existing unrelated dirty changes were kept separate from this work.
- Implementation: Wrapped the date/time controls in a two-column row. For expense entries, wrapped payment method, major category, and category controls in a three-column row. Income entries keep their existing category pair. Added field-row CSS that keeps controls on the requested row while preserving narrow-field sizing.
- Verification: Ran `cmd /c npm run build` in `frontend` successfully. Verified no TypeScript SFC/script or `.ts`/`.tsx` files. Ran `git diff --check -- frontend/src/components/CalendarWorkspace.vue frontend/src/style.css` with no whitespace errors. Confirmed the existing v-model and emit bindings for date, time, title, payment method, category group, category detail, memo, amount shortcuts, submit, edit, and delete are still present.
- Result: The quick entry form now lays out date/time and payment/category controls side by side without changing the existing ledger behavior.

### 2026-04-25 - Light and dark dashboard/map theme cohesion

- User request: Fix the mismatch between light mode and dark mode so grid elements follow dark mode correctly, and make the map feel natural in light mode as well, with both themes verified.
- Request analysis: The issue was mostly presentation-layer drift: map canvas/toolbar/search controls used hard-coded light surfaces, Leaflet map tiles were not adjusted per theme, and several dashboard/grid guide surfaces used fixed purple or bright accent colors instead of shared theme tokens. The backend and feature logic did not need to change.
- Actions taken: Checked `codingconvention.md`, reviewed global theme tokens in `style.css`, travel map styles, household calendar layout grid styles, main dashboard grid styles, and palette GridStack guide styles.
- Implementation: Added shared theme variables for grid guides and travel map surfaces, including canvas, tile filter, toolbar, control, and hint colors. Wired travel map canvas, Leaflet controls, search panels, toolbar buttons, and hints to those variables. Updated household calendar, main dashboard, and palette grid guides to use the same token pair. Softened dark-mode dashboard highlight palettes so cards no longer look like light-mode blocks, and added dark-mode surface coverage for calendar layout panels, drive cards, and family media cards.
- Verification: Ran `cmd /c npm run build` in `frontend` successfully. Verified no TypeScript SFC/script or `.ts`/`.tsx` files with `rg -n 'lang="ts"|lang=''ts''' frontend/src`, `rg --files frontend/src | rg '\.(ts|tsx)$'`, and `Get-ChildItem -Path frontend/src -Recurse -Include *.ts,*.tsx`. Ran `git diff --check -- frontend/src/style.css frontend/src/components/MainDashboardWorkspace.vue frontend/src/features/palette/components/DragDropGrid.vue` with no whitespace errors. Started the Vite dev server for browser verification, confirmed the local app responded with HTTP 200 and loaded in the in-app browser, then stopped the verification server.
- Result: Light and dark themes now share consistent grid and map tokens, dark-mode grid/dashboard surfaces stay visually integrated with the dark palette, and the light-mode map is toned down through a matching canvas, controls, and tile filter without changing application behavior.

### 2026-04-25 - Travel cluster single count badge

- User request: In the travel map cluster marker, remove the duplicated count shown at the top-left and bottom-right so the count appears only on the right side.
- Request analysis: `TravelMyMapClusterPanel.vue` rendered both `travel-cluster-pin__count` and `travel-cluster-pin__group` for aggregate clusters, while CSS positioned one at the lower right and the other at the upper left. Removing the group badge keeps the existing cluster count behavior while eliminating the duplicate visual number.
- Actions taken: Checked `codingconvention.md`, reviewed the cluster icon builder and related marker CSS, and confirmed no other references remain for the group badge.
- Implementation: Removed the aggregate-only `travel-cluster-pin__group` markup and the unused member count variable. Simplified the cluster badge CSS so only `travel-cluster-pin__count` is styled and displayed at the lower-right side of the marker.
- Verification: Ran `cmd /c npm run build` in `frontend` successfully. Confirmed `travel-cluster-pin__group` and `memberCount` no longer exist with `rg`. Verified no TypeScript SFC/script or `.ts`/`.tsx` files with `rg -n 'lang="ts"|lang=''ts''' frontend/src` and `Get-ChildItem -Path frontend/src -Recurse -Include *.ts,*.tsx`. Ran `git diff --check -- frontend/src/components/TravelMyMapClusterPanel.vue frontend/src/style.css worklog.md` with no whitespace errors.
- Result: Travel map cluster markers now show a single count badge at the lower-right side, eliminating the duplicated upper-left count without changing cluster selection or popup logic.

### 2026-04-25 - Household calendar no-scroll layout and aggregate hiding

- User request: Remove scrollbars from the calendar ledger layout, allow the user-configured aggregate panel to be hidden, and keep the month calendar shape intact even when its layout panel is resized smaller.
- Request analysis: The visible scrollbar came from layout panels allowing internal overflow while the month calendar retained a fixed minimum width/height. The aggregate panel already had an enable state, but it only disabled content inside the panel rather than hiding the panel itself.
- Actions taken: Checked `codingconvention.md`, reviewed `CalendarWorkspace.vue` layout state, GridStack snapshot persistence, aggregate panel controls, and the calendar layout CSS.
- Implementation: Filtered the aggregate panel out of the GridStack layout when aggregate display is turned off, added a top layout-toolbar button to hide/show it, and preserved hidden-panel layout data when saving visible panel snapshots. Changed calendar layout panels to hide internal overflow, made the month calendar fill its panel without horizontal or vertical scrollbars, and added container-query compaction so the 7-column month grid remains intact while metrics/details progressively reduce in tighter panels.
- Verification: Ran `cmd /c npm run build` in `frontend` successfully. Verified no TypeScript SFC/script or `.ts`/`.tsx` files with `rg -n 'lang="ts"|lang=''ts''' frontend/src` and `Get-ChildItem -Path frontend/src -Recurse -Include *.ts,*.tsx`. Ran `git diff --check -- frontend/src/components/CalendarWorkspace.vue frontend/src/style.css` with no whitespace errors.
- Result: The calendar ledger layout no longer shows internal scrollbars in its draggable panels, the user aggregate panel can be hidden/restored from the layout toolbar, and the month calendar keeps its 7-column shape when resized smaller.
